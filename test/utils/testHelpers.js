...
class TestHelper {
	...
	static fromBN(bn, decimal = 18) {
		return parseFloat(ethers.utils.formatUnits(bn.toString(), decimal));
	}
	...
	static getDebtTokenFromColl(coll, price) {
		return coll.mul(price).div(MoneyValues._1e18BN).toString();
	}
	 ...
	 */
	static async getOpenVesselTotalDebt(contracts, DebtTokenAmount, asset) {
		if (!asset) asset = this.ZERO_ADDRESS
		const fee = await contracts.vesselManager.getBorrowingFee(asset, DebtTokenAmount)
		const compositeDebt = await this.getCompositeDebt(contracts, DebtTokenAmount, asset)
	 ...
	 */
	static async getOpenVesselDebtTokenAmount(contracts, totalDebt, asset) {
		if (!asset) asset = this.ZERO_ADDRESS
	...
	static async getAmountWithBorrowingFee(contracts, DebtTokenAmount, asset) {
		if (!asset) asset = this.ZERO_ADDRESS
		const fee = await contracts.vesselManager.getBorrowingFee(asset, DebtTokenAmount)
	...
	static getEmittedRedemptionValues(redemptionTx) {
		for (let i = 0; i < redemptionTx.logs.length; i++) {
			if (redemptionTx.logs[i].event === "Redemption") {
				const LNDRAmount = redemptionTx.logs[i].args[1]
				...
				return [LNDRAmount, totalGRAIRedeemed, totalAssetDrawn, ETHFee]
			}
		}
	...
	static async openVessel_allAccounts(accounts, contracts, ETHAmount, LNDRAmount) {
		const gasCostList = []
		const totalDebt = await this.getOpenVesselTotalDebt(contracts, LNDRAmount)
	...
	static async openVessel_allAccounts_randomGRAI(
		...
		for (const account of accounts) {
			const randLNDRAmount = this.randAmountInWei(minGRAI, maxGRAI)
			const totalDebt = await this.getOpenVesselTotalDebt(contracts, randLNDRAmount)
			const { upperHint, lowerHint } = await this.getBorrowerOpsListHint(
				contracts,
				ETHAmount,
				totalDebt
	...
	static async openVessel_allAccounts_decreasingLNDRAmounts(
		accounts,
		contracts,
		ETHAmount,
		maxLNDRAmount
		...
		let i = 0
		for (const account of accounts) {
			const LNDRAmount = (maxLNDRAmount - i).toString()
			const LNDRAmountWei = web3.utils.toWei(LNDRAmount, "ether")
			const totalDebt = await this.getOpenVesselTotalDebt(contracts, LNDRAmountWei)
			const { upperHint, lowerHint } = await this.getBorrowerOpsListHint(
				contracts,
				ETHAmount,
				totalDebt
	...
	static async openVessel(
		...
		else if (typeof extraDebtTokenAmount == "string") extraDebtTokenAmount = this.toBN(extraDebtTokenAmount)
		...
		// console.log("debtTokenAmount", ethers.utils.formatUnits(debtTokenAmount.toString(), 18));
		...
		const totalDebt = await this.getOpenVesselTotalDebt(contracts, debtTokenAmount, asset)
	...
	static async withdrawDebtToken(
		contracts,
		{ asset, LNDRAmount, ICR, upperHint, lowerHint, extraParams }
	) {
		if (!asset) asset = this.ZERO_ADDRESS
		if (!upperHint) upperHint = this.ZERO_ADDRESS
		if (!lowerHint) lowerHint = this.ZERO_ADDRESS
		...
		if (ICR) {
			...
			LNDRAmount = await this.getNetBorrowingAmount(contracts, increasedTotalDebt)
		} else {
			increasedTotalDebt = await this.getAmountWithBorrowingFee(contracts, LNDRAmount)
		}
	...
	static async adjustVessel_allAccounts(accounts, contracts, ETHAmount, LNDRAmount) {
		const gasCostList = []
		...
		for (const account of accounts) {
			...
			let GRAIChangeBN = this.toBN(LNDRAmount)
	...
	static async withdrawGRAI_allAccounts_randomAmount(min, max, accounts, contracts) {
		...
		for (const account of accounts) {
			const randLNDRAmount = this.randAmountInWei(min, max)
	...
	static async redeemCollateral_allAccounts_randomAmount(
		...
		for (const redeemer of accounts) {
			...
			await this.performRedemptionTx(redeemer, price, contracts, randDebtTokenAmount, asset)
			const gas = this.gasUsed(tx)
			gasCostList.push(gas)
		}
		return this.getGasMetrics(gasCostList)
	...
	static async performRedemptionTx(redeemer, price, contracts, debtTokenAmount, asset, maxFee = 0) {
		if (!asset) asset = this.ZERO_ADDRESS
	...
	// --- StakedDebtToken gas functions ---

	static async provideToSP_allAccounts(accounts, stakedDebtToken, amount) {
		const gasCostList = []
		for (const account of accounts) {
			const tx = await stakedDebtToken.deposit(amount, this.ZERO_ADDRESS, { from: account })
			const gas = this.gasUsed(tx)
			gasCostList.push(gas)
		}
		return this.getGasMetrics(gasCostList)
	...
	static async provideToSP_allAccounts_randomAmount(min, max, accounts, stakedDebtToken) {
		const gasCostList = []
		for (const account of accounts) {
			const randomLNDRAmount = this.randAmountInWei(min, max)
			const tx = await stakedDebtToken.deposit(randomLNDRAmount, this.ZERO_ADDRESS, {
				from: account,
			})
			const gas = this.gasUsed(tx)
			gasCostList.push(gas)
	...
	static async withdrawFromSP_allAccounts(accounts, stakedDebtToken, amount) {
		const gasCostList = []
		for (const account of accounts) {
			const tx = await stakedDebtToken.withdraw(amount, { from: account })
			const gas = this.gasUsed(tx)
			gasCostList.push(gas)
		}
		return this.getGasMetrics(gasCostList)
	...
	static async withdrawFromSP_allAccounts_randomAmount(min, max, accounts, stakedDebtToken) {
		const gasCostList = []
		for (const account of accounts) {
			const randomLNDRAmount = this.randAmountInWei(min, max)
			const tx = await stakedDebtToken.withdraw(randomLNDRAmount, { from: account })
			const gas = this.gasUsed(tx)
			gasCostList.push(gas)
		}
		return this.getGasMetrics(gasCostList)
	...
	// 		console.log(`entireColl: ${entireColl}`)
	// 		console.log(`entireDebt: ${entireDebt}`)
	// 		const AssetGain = await contracts.stakedDebtToken.getDepositorETHGain(account)
	...
	// 		const tx = await contracts.stakedDebtToken.withdrawETHGainToVessel(upperHint, lowerHint, {
	// 			from: account,
	// 		})
	// 		const gas = this.gasUsed(tx)
	// 		gasCostList.push(gas)
