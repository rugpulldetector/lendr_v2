...
contract("Access Control: functions where the caller is restricted to Lendr contract(s)", async accounts => {
	...
	describe("ActivePool", async accounts => {
		...
		it("increaseDebt(): reverts when called by an account that is not BO nor VesselM", async () => {
			...
			try {
				...
				assert.include(err.message, "revert")
				// assert.include(err.message, "Caller is not a Lendr contract")
			}
		})
    ...
    // I will do it later on, coz I didn't complete communityIssuance.
	...
	describe("LNDRStaking", async accounts => {
		it("increaseFee_DebtToken(): reverts when caller is not VesselManager", async () => {
			try {
				const txAlice = await lndrStaking.increaseFee_DebtToken(debtToken.address, dec(1, 18), { from: alice })
			} catch (err) {
				assert.include(err.message, "revert")
			}
		})
