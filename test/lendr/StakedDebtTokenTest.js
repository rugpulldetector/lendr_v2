...
const { dec, toBN, fromBN, assertRevert } = th;
...
contract("StakedDebtToken", async (accounts) => {
  const [
    owner,
    defaulter_1,
    defaulter_2,
  ...
  async function _openVesselWithICR(
    erc20Contract,
    extraDebtTokenAmt,
    icr,
    sender
    ...
    await th.openVessel(contracts.core, {
      ...
      extraDebtTokenAmount: toBN(dec(extraDebtTokenAmt, 18)),
      ICR: toBN(dec(icr, 18)),
      extraParams: { from: sender },
    });
  }

  async function _openVesselWithCollAmt(
    erc20Contract,
    collAmt,
    extraDebtTokenAmt,
    sender
  ...
  async function openWhaleVessel(
    erc20Contract,
    icr = 2,
    extraDebtTokenAmt = 100_000
  ) {
    await openVessel({
      asset: erc20Contract.address,
  ...
  describe("StakedDebtToken Mechanisms", async () => {
    before(async () => {
    ...
    describe("Providing", async () => {
      it("deposit(): increases the StakedDebtToken balance", async () => {
        await _openVessel(erc20, (extraDebtTokenAmt = 200), alice);

        console.log("total Supply", await stakedDebtToken.convertToShares(200));

        await debtToken.approve(stakedDebtToken.address, 200, { from: alice });
        await stakedDebtToken.deposit(200, alice, { from: alice });

        console.log("total Supply", await stakedDebtToken.totalSupply());
        assert.equal(await stakedDebtToken.totalAssets(), 200);
      });

      it("deposit(): reverts when trying to make a SP deposit without debt token balance", async () => {
        const aliceTxPromise = stakedDebtToken.deposit(200, alice, {
          from: alice,
        });
        await assertRevert(aliceTxPromise, "revert");
      });

      it("deposit(): updates the user's deposit record in StakedDebtToken", async () => {
        await _openVessel(erc20, (extraDebtTokenAmt = 200), alice);
        assert.equal(await stakedDebtToken.balanceOf(alice), 0);

        await debtToken.approve(stakedDebtToken.address, 200, { from: alice });
        await stakedDebtToken.deposit(200, alice, { from: alice });

        assert.equal(await stakedDebtToken.balanceOf(alice), 200);
      });

      it("deposit(): reduces the user's debt token balance by the correct amount", async () => {
        await _openVessel(erc20, (extraDebtTokenAmt = 200), alice);
        const alice_balance_Before = await debtToken.balanceOf(alice);

        await debtToken.approve(stakedDebtToken.address, 400, { from: alice });
        await stakedDebtToken.deposit(400, alice, { from: alice });

        const alice_balance_After = await debtToken.balanceOf(alice);
        assert.equal(alice_balance_Before.sub(alice_balance_After), "400");
      });

      // it("deposit(): Correctly updates user snapshots of accumulated rewards per unit staked", async () => {
      //   await openWhaleVessel(erc20);
      //   await openWhaleVessel(erc20B);

      //   const whaleDebtTokenBalance = await debtToken.balanceOf(whale);
      //   await stakedDebtToken.provideToSP(
      //     whaleDebtTokenBalance.div(toBN(2)),
      //     validCollateral,
      //     { from: whale }
      //   );
      ...
      //   await _openVesselWithICR(
      //     erc20,
      //     (extraDebtTokenAmt = 100),
      //     (icr = 5),
      //     alice
      //   );
      //   await _openVesselWithICR(
      ...
      //   const debtTokenDepositsBefore =
      //     await stakedDebtToken.getTotalDebtTokenDeposits();
      ...
      //   );
      //   assert.isFalse(
      //     await sortedVessels.contains(erc20B.address, defaulter_2)
      //   );

      //   // Confirm SP has decreased
      //   const debtTokenDepositsAfter =
      //     await stakedDebtToken.getTotalDebtTokenDeposits();
      //   assert.isTrue(debtTokenDepositsAfter.lt(debtTokenDepositsBefore));

      //   const P_Before = await stakedDebtToken.P();
      //   const S_Before_ERC1 = await stakedDebtToken.epochToScaleToSum(
      //     erc20.address,
      //     0,
      //     0
      //   );
      //   const S_Before_ERC2 = await stakedDebtToken.epochToScaleToSum(
      //     erc20B.address,
      //     0,
      //     0
      //   );
      //   const G_Before = await stakedDebtToken.epochToScaleToG(0, 0);
      ...
      //   const alice_snapshot_Before = await stakedDebtToken.depositSnapshots(
      //     alice
      //   );
      //   const alice_snapshot_S_Before_ERC1 = await stakedDebtToken.S(
      //     alice,
      //     erc20.address
      //   );
      //   const alice_snapshot_S_Before_ERC2 = await stakedDebtToken.S(
      //     alice,
      //     erc20B.address
      //   );
      ...
      //   // Make deposit
      //   await stakedDebtToken.provideToSP(dec(100, 18), validCollateral, {
      //     from: alice,
      //   });

      //   // Check 'After' snapshots
      //   const alice_snapshot_After = await stakedDebtToken.depositSnapshots(
      //     alice
      //   );
      //   const alice_snapshot_S_After_A = await stakedDebtToken.S(
      //     alice,
      //     erc20.address
      //   );
      //   const alice_snapshot_S_After_B = await stakedDebtToken.S(
      //     alice,
      //     erc20B.address
      //   );
      ...
      //   assert.equal(
      //     alice_snapshot_S_After_A.toString(),
      //     S_Before_ERC1.toString()
      //   );
      //   assert.equal(
      //     alice_snapshot_S_After_B.toString(),
      //     S_Before_ERC2.toString()
      //   );
      //   assert.equal(alice_snapshot_P_After, P_Before);
      //   assert.equal(alice_snapshot_G_After, G_Before);
      // });

      // it("deposit(): multiple deposits = updates user's deposit and snapshots", async () => {
      //   await openWhaleVessel(erc20);
      //   const whaleDebtTokenBalance = await debtToken.balanceOf(whale);

      //   await debtToken.approve(stakedDebtToken.address, whaleDebtTokenBalance.div(toBN(2)), { from: whale });
      //   await stakedDebtToken.deposit(
      //     whaleDebtTokenBalance.div(toBN(2)),
      //     whale,
      //     { from: whale }
      //   );
      ...
      //     from: alice,
      //   });
      //   await stakedDebtToken.deposit(dec(150, 18), alice, {
      //     from: alice,
      //   });
      ...
      //     from: dennis,
      //   });
      //   await stakedDebtToken.deposit(dec(150, 18), dennis, {
      //     from: dennis,
      //   });

      //   const alice_Snapshot_0 = await stakedDebtToken.depositSnapshots(alice);
      //   const alice_Snapshot_S_0_ERC1 = await stakedDebtToken.S(
      //     alice,
      //     erc20.address
      //   );
      ...
      //   const dennis_Snapshot_0 = await stakedDebtToken.depositSnapshots(dennis);
      //   const dennis_Snapshot_S_0_ERC2 = await stakedDebtToken.S(
      //     dennis,
      //     erc20B.address
      //   );
      ...
      //     from: owner,
      //   });
      //   await vesselManagerOperations.liquidate(erc20B.address, defaulter_4, {
      //     from: owner,
      //   });
      //   await vesselManagerOperations.liquidate(erc20B.address, defaulter_5, {
      //     from: owner,
      //   });
      ...
      //   await stakedDebtToken.provideToSP(alice_topUp_1, validCollateral, {
      //     from: alice,
      //   });
      //   const alice_newDeposit_1 = (
      //     await stakedDebtToken.deposit(alice)
      //   ).toString();
      //   assert.equal(
      ...
      //   const dennis_compoundedDeposit_1 =
      //     await stakedDebtToken.getCompoundedDebtTokenDeposits(dennis);
      //   const dennis_topUp_1 = toBN(dec(100, 18));
      //   await stakedDebtToken.provideToSP(dennis_topUp_1, validCollateral, {
      //     from: dennis,
      //   });
      //   const dennis_newDeposit_1 = (
      //     await stakedDebtToken.deposit(dennis)
      //   ).toString();
      //   assert.equal(
      //     dennis_compoundedDeposit_1.add(dennis_topUp_1),
      //     dennis_newDeposit_1
      //   );

      //   // get system reward terms
      //   const P_1 = await stakedDebtToken.P();
      //   const S_1_ERC1 = await stakedDebtToken.epochToScaleToSum(
      //     erc20.address,
      //     0,
      //     0
      //   );
      //   const S_1_ERC2 = await stakedDebtToken.epochToScaleToSum(
      //     erc20B.address,
      //     0,
      //     0
      //   );
      ...
      //   const alice_Snapshot_1 = await stakedDebtToken.depositSnapshots(alice);
      //   const alice_Snapshot_S_1 = await stakedDebtToken.S(alice, erc20.address);
      ...
      //   const dennis_Snapshot_1 = await stakedDebtToken.depositSnapshots(dennis);
      //   const dennis_Snapshot_S_1 = await stakedDebtToken.S(
      //     dennis,
      //     erc20B.address
      //   );
      ...
      //   // Bob and Erin open vessels and deposit to StakedDebtToken
      //   await _openVessel(erc20, (extraDebtTokenAmt = 1_000), bob);
      //   await stakedDebtToken.provideToSP(dec(800, 18), validCollateral, {
      //     from: bob,
      //   });
      ...
      //     from: owner,
      //   });
      //   await vesselManagerOperations.liquidate(erc20B.address, defaulter_6, {
      //     from: owner,
      //   });

      //   const P_2 = await stakedDebtToken.P();
      //   const S_2_ERC1 = await stakedDebtToken.epochToScaleToSum(
      //     erc20.address,
      //     0,
      //     0
      //   );
      //   const S_2_ERC2 = await stakedDebtToken.epochToScaleToSum(
      //     erc20B.address,
      //     0,
      //     0
      //   );
      ...
      //   const alice_Snapshot_2 = await stakedDebtToken.depositSnapshots(alice);
      //   const alice_Snapshot_S_2 = await stakedDebtToken.S(alice, erc20.address);
      ...
      //   const dennis_Snapshot_2 = await stakedDebtToken.depositSnapshots(dennis);
      //   const dennis_Snapshot_S_2 = await stakedDebtToken.S(
      //     dennis,
      //     erc20B.address
      //   );
      ...
      it("deposit(): reverts if user tries to provide more than their debt token balance", async () => {
        ...
        await debtToken.approve(
          stakedDebtToken.address,
          aliceBal.add(toBN(1)),
          {
            from: alice,
          }
        );
        const aliceTxPromise = stakedDebtToken.deposit(
          aliceBal.add(toBN(1)),
          alice,
          { from: alice }
        );
        ...
        await debtToken.approve(
          stakedDebtToken.address,
          bobBal.add(toBN(dec(235534, 18))),
          {
            from: bob,
          }
        );

        const bobTxPromise = stakedDebtToken.deposit(
          bobBal.add(toBN(dec(235534, 18))),
          bob,
          {
            from: bob,
      ...
      it("deposit(): reverts if user tries to provide 2^256-1 debt tokens, which exceeds their balance", async () => {
        await _openVessel(erc20, 10_000, alice);
        ...
        try {
          await debtToken.approve(stakedDebtToken.address, maxBytes32, {
            from: alice,
          });

          aliceTx = await stakedDebtToken.deposit(maxBytes32, alice, {
            from: alice,
          });
      ...
      // it("deposit(): doesn't impact other users' deposits or collateral gains", async () => {
      //   await openWhaleVessel(erc20);
      //   await openWhaleVessel(erc20B);
      ...
      //   await debtToken.approve(stakedDebtToken.address, dec(1_000, 18), {
      //     from: alice,
      //   });
      //   await stakedDebtToken.deposit(dec(1_000, 18), alice, {
      //     from: alice,
      //   });

      //   await debtToken.approve(stakedDebtToken.address, dec(2_000, 18), {
      //     from: bob,
      //   });

      //   await stakedDebtToken.deposit(dec(2_000, 18), bob, {
      //     from: bob,
      //   });

      //   await debtToken.approve(stakedDebtToken.address, dec(3_000, 18), {
      //     from: carol,
      //   });
      //   await stakedDebtToken.deposit(dec(3_000, 18), carol, {
      //     from: carol,
      //   });
      ...
      //   const poolDebtBalance =
      //     await stakedDebtToken.getTotalDebtTokenDeposits();
      //   const poolCollBalance = await stakedDebtToken.getCollateral(
      //     erc20.address
      //   );
      ...
      //     from: dennis,
      //   });
      //   await stakedDebtToken.deposit(dec(1_000, 18), dennis, {
      //     from: dennis,
      //   });
      //   assert.equal(
      //     (
      ...
      //     await stakedDebtToken.getCompoundedDebtTokenDeposits(bob)
      //   ).toString();
      //   const carol_deposit_After = (
      //     await stakedDebtToken.getCompoundedDebtTokenDeposits(carol)
      //   ).toString();

      //   const alice_gain_After = (
      //     await stakedDebtToken.getDepositorGains(alice, validCollateral)
      //   )[1].toString();
      //   const bob_gain_After = (
      //     await stakedDebtToken.getDepositorGains(bob, validCollateral)
      //   )[1].toString();
      //   const carol_gain_After = (
      //     await stakedDebtToken.getDepositorGains(carol, validCollateral)
      //   )[1].toString();
      ...
      // it("deposit(): doesn't impact system debt, collateral or TCR", async () => {
      //   await openWhaleVessel(erc20);
      ...
      //   await stakedDebtToken.provideToSP(dec(1_000, 18), validCollateral, {
      //     from: alice,
      //   });
      //   await stakedDebtToken.provideToSP(dec(2_000, 18), validCollateral, {
      //     from: bob,
      //   });
      //   await stakedDebtToken.provideToSP(dec(3_000, 18), validCollateral, {
      //     from: carol,
      //   });
      ...
      //   const activeDebt_AfterERC20 = (
      //     await activePool.getDebtTokenBalance(erc20.address)
      //   ).toString();
      //   const defaultedDebt_AfterERC20 = (
      //     await defaultPool.getDebtTokenBalance(erc20.address)
      //   ).toString();
      //   const activeColl_AfterERC20 = (
      //     await activePool.getAssetBalance(erc20.address)
      //   ).toString();
      //   const defaultedColl_AfterERC20 = (
      //     await defaultPool.getAssetBalance(erc20.address)
      //   ).toString();
      //   const TCR_AfterERC20 = (
      //     await th.getTCR(contracts.core, erc20.address)
      //   ).toString();
      ...
      // it("deposit(): doesn't impact any vessels, including the caller's vessel", async () => {
      //   await openWhaleVessel(erc20);
      ...
      //   const whale_Debt_BeforeERC20 = (
      //     await vesselManager.Vessels(whale, erc20.address)
      //   )[0].toString();
      //   const alice_Debt_BeforeERC20 = (
      //     await vesselManager.Vessels(alice, erc20.address)
      //   )[0].toString();
      //   const bob_Debt_BeforeERC20 = (
      //     await vesselManager.Vessels(bob, erc20.address)
      //   )[0].toString();
      //   const carol_Debt_BeforeERC20 = (
      //     await vesselManager.Vessels(carol, erc20.address)
      //   )[0].toString();
      //   const dennis_Debt_BeforeERC20 = (
      //     await vesselManager.Vessels(dennis, erc20.address)
      //   )[0].toString();

      //   const whale_Coll_BeforeERC20 = (
      //     await vesselManager.Vessels(whale, erc20.address)
      //   )[th.VESSEL_COLL_INDEX].toString();
      //   const alice_Coll_BeforeERC20 = (
      //     await vesselManager.Vessels(alice, erc20.address)
      //   )[th.VESSEL_COLL_INDEX].toString();
      //   const bob_Coll_BeforeERC20 = (
      //     await vesselManager.Vessels(bob, erc20.address)
      //   )[th.VESSEL_COLL_INDEX].toString();
      //   const carol_Coll_BeforeERC20 = (
      //     await vesselManager.Vessels(carol, erc20.address)
      //   )[th.VESSEL_COLL_INDEX].toString();
      //   const dennis_Coll_BeforeERC20 = (
      //     await vesselManager.Vessels(dennis, erc20.address)
      //   )[th.VESSEL_COLL_INDEX].toString();

      //   const whale_ICR_BeforeERC20 = (
      //     await vesselManager.getCurrentICR(erc20.address, whale, price)
      //   ).toString();
      //   const alice_ICR_BeforeERC20 = (
      //     await vesselManager.getCurrentICR(erc20.address, alice, price)
      //   ).toString();
      //   const bob_ICR_BeforeERC20 = (
      //     await vesselManager.getCurrentICR(erc20.address, bob, price)
      //   ).toString();
      //   const carol_ICR_BeforeERC20 = (
      //     await vesselManager.getCurrentICR(erc20.address, carol, price)
      //   ).toString();
      //   const dennis_ICR_BeforeERC20 = (
      //     await vesselManager.getCurrentICR(erc20.address, dennis, price)
      //   ).toString();
      ...
      //   const whale_Debt_AfterERC20 = (
      //     await vesselManager.Vessels(whale, erc20.address)
      //   )[0].toString();
      //   const alice_Debt_AfterERC20 = (
      //     await vesselManager.Vessels(alice, erc20.address)
      //   )[0].toString();
      //   const bob_Debt_AfterERC20 = (
      //     await vesselManager.Vessels(bob, erc20.address)
      //   )[0].toString();
      //   const carol_Debt_AfterERC20 = (
      //     await vesselManager.Vessels(carol, erc20.address)
      //   )[0].toString();
      //   const dennis_Debt_AfterERC20 = (
      //     await vesselManager.Vessels(dennis, erc20.address)
      //   )[0].toString();

      //   const whale_Coll_AfterERC20 = (
      //     await vesselManager.Vessels(whale, erc20.address)
      //   )[th.VESSEL_COLL_INDEX].toString();
      //   const alice_Coll_AfterERC20 = (
      //     await vesselManager.Vessels(alice, erc20.address)
      //   )[th.VESSEL_COLL_INDEX].toString();
      //   const bob_Coll_AfterERC20 = (
      //     await vesselManager.Vessels(bob, erc20.address)
      //   )[th.VESSEL_COLL_INDEX].toString();
      //   const carol_Coll_AfterERC20 = (
      //     await vesselManager.Vessels(carol, erc20.address)
      //   )[th.VESSEL_COLL_INDEX].toString();
      //   const dennis_Coll_AfterERC20 = (
      //     await vesselManager.Vessels(dennis, erc20.address)
      //   )[th.VESSEL_COLL_INDEX].toString();
      ...
      // it("deposit(): doesn't protect the depositor's vessel from liquidation", async () => {
      //   // A, B, C open vessels and make Stability Pool deposits
      //   await _openVessel(erc20, 1_000, alice);
      //   await _openVessel(erc20, 2_000, bob);
      //   await _openVessel(erc20, 3_000, carol);
      ...
      //   assert.equal(
      //     (await vesselManager.getVesselStatus(erc20.address, bob)).toString(),
      //     "1"
      //   );

      //   // Confirm Bob has a Stability deposit
      //   assert.equal(
      //     (await stakedDebtToken.getCompoundedDebtTokenDeposits(bob)).toString(),
      //     dec(1_000, 18)
      //   );
      ...
      // it("deposit(): providing 0 reverts", async () => {
      //   // A, B, C open vessels and make Stability Pool deposits
      //   await _openVessel(erc20, 1_000, alice);
      //   await _openVessel(erc20, 2_000, bob);
      //   await _openVessel(erc20, 3_000, carol);
      ...
      //     from: alice,
      //   });
      //   await stakedDebtToken.provideToSP(dec(50, 18), validCollateral, {
      //     from: bob,
      //   });
      //   await stakedDebtToken.provideToSP(dec(30, 18), validCollateral, {
      //     from: carol,
      //   });
      ...
      //   const txPromise_B = stakedDebtToken.provideToSP(0, validCollateral, {
      //     from: bob,
      //   });
      //   await th.assertRevert(txPromise_B);
      // });

      // it("deposit(): new deposit = when SP > 0, triggers GRVT reward event - increases the sum G", async () => {
      //   // A, B, C open vessels and make Stability Pool deposits
      //   await _openVessel(erc20, 1_000, alice);
      //   await _openVessel(erc20, 2_000, bob);
      //   await _openVessel(erc20, 3_000, carol);
      ...
      //   let currentEpoch = await stakedDebtToken.currentEpoch();
      //   let currentScale = await stakedDebtToken.currentScale();
      //   const G_Before = await stakedDebtToken.epochToScaleToG(
      //     currentEpoch,
      //     currentScale
      //   );
      ...
      //   currentEpoch = await stakedDebtToken.currentEpoch();
      //   currentScale = await stakedDebtToken.currentScale();
      //   const G_After = await stakedDebtToken.epochToScaleToG(
      //     currentEpoch,
      //     currentScale
      //   );
      ...
      // it("deposit(): new deposit when SP is empty, doesn't update G", async () => {
      //   await _openVessel(erc20, 1_000, alice);
      //   await _openVessel(erc20, 2_000, bob);
      //   await _openVessel(erc20, 3_000, carol);
      ...
      //   // A withdraws
      //   await stakedDebtToken.withdrawFromSP(dec(1_000, 18), validCollateral, {
      //     from: alice,
      //   });

      //   // Check SP is empty
      //   assert.equal(await stakedDebtToken.getTotalDebtTokenDeposits(), "0");
      ...
      // it("deposit(): new deposit = depositor does not receive any GRVT rewards", async () => {
      //   // A, B, open vessels
      //   await _openVessel(erc20, 1_000, alice);
      //   await _openVessel(erc20, 2_000, bob);
      ...
      // it("deposit(): new deposit after past full withdrawal = depositor does not receive any GRVT rewards", async () => {
      ...
      //   const initialDeposit_A = (await debtToken.balanceOf(alice)).div(
      //     toBN(2)
      //   );
      //   const initialDeposit_B = (await debtToken.balanceOf(bob)).div(toBN(2));
      //   await stakedDebtToken.provideToSP(initialDeposit_A, validCollateral, {
      //     from: alice,
      //   });
      //   await stakedDebtToken.provideToSP(initialDeposit_B, validCollateral, {
      //     from: bob,
      //   });
      ...
      //   await stakedDebtToken.provideToSP(dec(5, 18), validCollateral, {
      //     from: carol,
      //   });
      ...
      //   await stakedDebtToken.withdrawFromSP(initialDeposit_A, validCollateral, {
      //     from: alice,
      //   });
      //   await stakedDebtToken.withdrawFromSP(initialDeposit_B, validCollateral, {
      //     from: bob,
      //   });
      ...
      //     from: alice,
      //   });
      //   await stakedDebtToken.provideToSP(dec(200, 18), validCollateral, {
      //     from: bob,
      //   });
      ...
      // it("deposit(): new deposit = depositor does not receive gains", async () => {
      //   await openWhaleVessel(erc20);
      ...
      //     from: bob,
      //   });
      //   await stakedDebtToken.provideToSP(dec(300, 18), validCollateral, {
      //     from: carol,
      //   });
      //   await stakedDebtToken.provideToSP(dec(400, 18), validCollateral, {
      //     from: dennis,
      //   });
      ...
      // it("deposit(): new deposit after past full withdrawal = depositor does not receive gains", async () => {
      //   await openWhaleVessel(erc20, (icr = 10));
      ...
      //   // A, B, C, D provide to SP
      //   await stakedDebtToken.provideToSP(dec(105, 18), validCollateral, {
      //     from: alice,
      //   });
      //   await stakedDebtToken.provideToSP(dec(105, 18), validCollateral, {
      //     from: bob,
      ...
      //   await stakedDebtToken.withdrawFromSP(dec(105, 18), validCollateral, {
      //     from: alice,
      //   });
      //   await stakedDebtToken.withdrawFromSP(dec(105, 18), validCollateral, {
      //     from: bob,
      ...
      //   assert.equal(
      //     A_Balance_AfterERC20.toString(),
      //     A_Balance_BeforeERC20.toString()
      //   );
      //   assert.equal(
      //     B_Balance_AfterERC20.toString(),
      //     B_Balance_BeforeERC20.toString()
      //   );
      //   assert.equal(
      //     C_Balance_AfterERC20.toString(),
      //     C_Balance_BeforeERC20.toString()
      //   );
      //   assert.equal(
      //     D_Balance_AfterERC20.toString(),
      //     D_Balance_BeforeERC20.toString()
      //   );
      // });

      // it("deposit(): topup = triggers GRVT reward event - increases the sum G", async () => {
      //   // A, B, C open vessels
      //   await _openVessel(erc20, 1_000, alice);
      //   await _openVessel(erc20, 2_000, bob);
      //   await _openVessel(erc20, 3_000, carol);
      ...
      //   const G_After = await stakedDebtToken.epochToScaleToG(0, 0);

      //   // Expect G has increased from the GRVT reward event triggered by B's topup
      //   assert.isTrue(G_After.gt(G_Before));
      // });

      // it("deposit(): topup = depositor receives GRVT rewards", async () => {
      //   // A, B, C open vessels
      ...
      //   // A, B, C, provide to SP
      //   await stakedDebtToken.provideToSP(dec(10, 18), validCollateral, {
      //     from: alice,
      //   });
      //   await stakedDebtToken.provideToSP(dec(20, 18), validCollateral, {
      //     from: bob,
      //   });
      //   await stakedDebtToken.provideToSP(dec(30, 18), validCollateral, {
      //     from: carol,
      ...
      // it("deposit(): passing same asset twice will revert", async () => {
      //   await openWhaleVessel(
      //     erc20,
      //     (icr = 10),
      //     (extraDebtTokenAmt = 1_000_000)
      //   );
      ...
      //   const txPromise = stakedDebtToken.provideToSP(
      //     dec(1_000, 18),
      //     [erc20.address, erc20B.address, erc20.address],
      //     {
      //       from: whale,
      //     }
      //   );
      //   await assertRevert(
      //     txPromise,
      //     "StakedDebtToken__ArrayNotInAscendingOrder"
      //   );
      // });

      // it("deposit(): passing asset array in non-ascending order will revert", async () => {
      //   await openWhaleVessel(
      //     erc20,
      //     (icr = 10),
      //     (extraDebtTokenAmt = 1_000_000)
      //   );
      //   // correct order
      //   await stakedDebtToken.provideToSP(dec(199_000, 18), validCollateral, {
      //     from: whale,
      //   });
      ...
      // it("deposit(): passing wrong address to asset list has no impact", async () => {
      //   await openWhaleVessel(
      //     erc20,
      //     (icr = 10),
      //     (extraDebtTokenAmt = 1_000_000)
      //   );
      //   // first call won't revert as there is no initial deposit
      //   await stakedDebtToken.provideToSP(dec(199_000, 18), [alice], {
      //     from: whale,
      //   });
      //   await stakedDebtToken.provideToSP(dec(1_000, 18), [alice], {
      //     from: whale,
      //   });
      //   await stakedDebtToken.withdrawFromSP(dec(1_000, 18), [alice], {
      //     from: whale,
      //   });
      // });

      // it("deposit(): reverts when amount is zero", async () => {
      //   await openWhaleVessel(erc20, (icr = 10));
      ...
      //   txPromise_A = stakedDebtToken.provideToSP(0, validCollateral, {
      //     from: alice,
      //   });
      //   txPromise_B = stakedDebtToken.provideToSP(0, validCollateral, {
      //     from: bob,
      //   });
      //   txPromise_C = stakedDebtToken.provideToSP(0, validCollateral, {
      //     from: carol,
      //   });
      //   txPromise_D = stakedDebtToken.provideToSP(0, validCollateral, {
      //     from: dennis,
      //   });

      //   await th.assertRevert(
      //     txPromise_A,
      //     "StakedDebtToken: Amount must be non-zero"
      //   );
      //   await th.assertRevert(
      //     txPromise_B,
      //     "StakedDebtToken: Amount must be non-zero"
    ...
    describe("Withdrawing", async () => {
      it("withdraw(): reverts when user has no active deposit", async () => {
        await _openVessel(erc20, 100, alice);
        ...
        await stakedDebtToken.deposit(dec(100, 18), alice, {
          from: alice,
        });

        const alice_initialDeposit = (
          await stakedDebtToken.balanceOf(alice)
        ).toString();
        const bob_initialDeposit = (
          await stakedDebtToken.balanceOf(bob)
        ).toString();
        ...
        try {
          const txBob = await stakedDebtToken.withdraw(dec(100, 18), bob, bob, {
            from: bob,
          });
      ...
      // it("withdraw(): partial retrieval - retrieves correct debt token amount and the entire collateral gain, and updates deposit", async () => {
      //   await openWhaleVessel(
      //     erc20,
      //     (icr = 10),
      //     (extraDebtTokenAmt = 1_000_000)
      //   );
      //   await stakedDebtToken.provideToSP(dec(185_000, 18), [], { from: whale });
      ...
      //   await debtToken.approve(stakedDebtToken.address, dec(15_000, 18), {from: alice});
      //   await stakedDebtToken.deposit(dec(15_000, 18), alice, {
      //     from: alice,
      //   });
      ...
      //   const liquidationTX_1 = await vesselManagerOperations.liquidate(
      //     erc20.address,
      //     defaulter_1,
      //     {
      //       from: owner,
      //     }
      //   ); // 170 closed
      //   const liquidationTX_2 = await vesselManagerOperations.liquidate(
      //     erc20.address,
      //     defaulter_2,
      //     {
      //       from: owner,
      ...
      //   const expectedCompoundedDeposit_A = toBN(dec(15_000, 18)).sub(
      //     expectedLoss_A
      //   );
      ...
      //   await stakedDebtToken.withdraw(dec(9_000, 18), alice, alice, {
      //     from: alice,
      //   });

      //   const expectedNewDeposit_A = compoundedDeposit_A.sub(
      //     toBN(dec(9_000, 18))
      //   );

      //   // check Alice's deposit has been updated to equal her compounded deposit minus her withdrawal
      //   const newDeposit = (await stakedDebtToken.balanceOf(alice)).toString();
      //   assert.isAtMost(
      ...
      //   const alice_pendingAssetGain = (
      //     await stakedDebtToken.getDepositorGains(alice, [erc20.address])
      //   )[1][0];
      //   assert.equal(alice_pendingAssetGain, 0);
      // });

      it("withdraw(): partial retrieval - leaves the correct amount of debt tokens in the Stability Pool", async () => {
        await openWhaleVessel(
          erc20,
          (icr = 10),
          (extraDebtTokenAmt = 1_000_000)
        );
        await debtToken.approve(stakedDebtToken.address, dec(185_000, 18), {
          from: whale,
        });
        await stakedDebtToken.deposit(dec(185_000, 18), whale, {
          from: whale,
        });
        ...
        // Alice makes deposit #1: 15_000
        await _openVesselWithICR(erc20, 15_000, (icr = 10), alice);
        await debtToken.approve(stakedDebtToken.address, dec(15_000, 18), {
          from: alice,
        });
        await stakedDebtToken.deposit(dec(15_000, 18), alice, {
          from: alice,
        ...
        const poolAssetsBefore = await stakedDebtToken.totalAssets();

        assert.equal(poolAssetsBefore, dec(200_000, 18));
        ...
        // 2 users liquidated
        const liquidationTx1 = await vesselManagerOperations.liquidate(
          erc20.address,
          defaulter_1,
          { from: owner }
        );
        const liquidationTx2 = await vesselManagerOperations.liquidate(
          erc20.address,
          defaulter_2,
          { from: owner }
        );

        const [liquidatedDebt1, liquidatedColl1, collGasComp1, GRAIGasComp1] =
          th.getEmittedLiquidationValues(liquidationTx1);
        const [liquidatedDebt2, liquidatedColl2, collGasComp2, GRAIGasComp2] =
          th.getEmittedLiquidationValues(liquidationTx2);

        const changedCollToDebt =
          (fromBN(liquidatedColl1) + fromBN(liquidatedColl2)) * fromBN(price);
        ...
        const expectedPoolAssets = toBN(dec(200_000, 18))
          .sub(toBN(liquidatedDebt1))
          .sub(toBN(liquidatedDebt2))
          .sub(toBN(dec(9_000, 18)))
          .add(toBN(th.getDebtTokenFromColl(liquidatedColl1, price)))
          .add(toBN(th.getDebtTokenFromColl(liquidatedColl2, price)));

        const poolAssetsAfter = (
          await stakedDebtToken.totalAssets()
        ).toString();

        console.log("poolAssetsAfter", fromBN(poolAssetsAfter));

        th.assertIsApproximatelyEqual(poolAssetsAfter, expectedPoolAssets);
      });

      it("withdraw(): full retrieval - leaves the correct amount of debt tokens in the Stability Pool", async () => {
        await openWhaleVessel(
          erc20,
          (icr = 10),
          (extraDebtTokenAmt = 1_000_000)
        );
        await stakedDebtToken.provideToSP(dec(185_000, 18), validCollateral, {
          from: whale,
        });
        ...
        await _openVesselWithICR(erc20, 15_000, (icr = 10), alice);
        await stakedDebtToken.provideToSP(dec(15_000, 18), validCollateral, {
          from: alice,
        });

        const poolDepositsBefore =
          await stakedDebtToken.getTotalDebtTokenDeposits();
        ...
        const [liquidatedDebt1] =
          th.getEmittedLiquidationValues(liquidationTx1);
        const [liquidatedDebt2] =
          th.getEmittedLiquidationValues(liquidationTx2);
        ...
        const expectedCompoundedDeposit = toBN(dec(15_000, 18)).sub(
          expectedLoss
        );
        ...
        assert.isAtMost(
          th.getDifference(expectedCompoundedDeposit, compoundedDeposit),
          100_000
        );

        const poolDeposits = await stakedDebtToken.getTotalDebtTokenDeposits();

        // Alice retrieves all of her entitled tokens
        await stakedDebtToken.withdrawFromSP(dec(15_000, 18), validCollateral, {
          from: alice,
        });

        const expectedPoolDeposits = poolDeposits.sub(compoundedDeposit);
        const poolDepositsAfter =
          await stakedDebtToken.getTotalDebtTokenDeposits();
        assert.isAtMost(
          th.getDifference(expectedPoolDeposits, poolDepositsAfter),
          100_000
        );
      });

      //   it("withdraw(): Subsequent deposit and withdrawal attempt from same account, with no intermediate liquidations, withdraws zero collateral", async () => {
      //     // --- SETUP ---
      //     await openWhaleVessel(
      //       erc20,
      //       (icr = 10),
      //       (extraDebtTokenAmt = 1_000_000)
      //     );
      //     await stakedDebtToken.provideToSP(dec(18_500, 18), validCollateral, {
      //       from: whale,
      //     });
      ...
      //     });
      //     await stakedDebtToken.provideToSP(dec(15000, 18), validCollateral, {
      //       from: alice,
      //     });
      ...
      //     await stakedDebtToken.withdrawFromSP(dec(15000, 18), validCollateral, {
      //       from: alice,
      //     });
      //     assert.equal(
      //       (await stakedDebtToken.getDepositorGains(alice, validCollateral))[1]
      //         .length,
      //       0
      //     );

      //     // Alice makes second deposit
      //     await stakedDebtToken.provideToSP(dec(10000, 18), validCollateral, {
      //       from: alice,
      //     });
      ...
      //     const ERC20inSP_Before = (
      //       await stakedDebtToken.getCollateral(erc20.address)
      //     ).toString();

      //     // Alice attempts second withdrawal
      //     await stakedDebtToken.withdrawFromSP(dec(10000, 18), validCollateral, {
      //       from: alice,
      //     });
      //     assert.equal(
      //       (await stakedDebtToken.getDepositorGains(alice, validCollateral))[1]
      ...
      //   it("withdraw(): it correctly updates the user's GRAI and collateral snapshots of entitled reward per unit staked", async () => {
      //     // --- SETUP ---
      //     // Whale deposits 185000 GRAI in StakedDebtToken
      //     await openWhaleVessel(
      //       erc20,
      //       (icr = 10),
      //       (extraDebtTokenAmt = 1_000_000)
      ...
      //       alice
      //     );
      //     const alice_snapshot_S_Before = await stakedDebtToken.S(
      //       alice,
      //       erc20.address
      ...
      //     await stakedDebtToken.withdrawFromSP(dec(9000, 18), validCollateral, {
      //       from: alice,
      //     });

      //     const P = (await stakedDebtToken.P()).toString();
      //     const S = (
      //       await stakedDebtToken.epochToScaleToSum(erc20.address, 0, 0)
      //     ).toString();
      //     // check 'After' snapshots
      //     const alice_snapshot_After = await stakedDebtToken.depositSnapshots(
      //       alice
      //     );
      //     const alice_snapshot_S_After = await stakedDebtToken.S(
      //       alice,
      //       erc20.address
      //     ); //alice_snapshot_After[0].toString()
      //     const alice_snapshot_P_After = alice_snapshot_After["P"].toString();
      //     assert.equal(alice_snapshot_S_After, S);
      //     assert.equal(alice_snapshot_P_After, P);
      //   });

      //   it("withdraw(): decreases StakedDebtToken ERC20", async () => {
      //     await openWhaleVessel(
      //       erc20,
      //       (icr = 10),
      //       (extraDebtTokenAmt = 1_000_000)
      ...
      //     await openVessel({
      //       asset: erc20.address,
      //       extraDebtTokenAmount: toBN(dec(15_000, 18)),
      //       ICR: toBN(dec(10, 18)),
      //       extraParams: { from: alice },
      //     });
      //     await stakedDebtToken.provideToSP(dec(15_000, 18), validCollateral, {
      ...
      //     const liquidationTx_1ERC20 = await vesselManagerOperations.liquidate(
      //       erc20.address,
      //       defaulter_1,
      //       {
      //         from: owner,
      //       }
      //     ); // 180 GRAI closed
      //     const [, liquidatedCollERC20] =
      //       th.getEmittedLiquidationValues(liquidationTx_1ERC20);

      //     // Get ActivePool and StakedDebtToken collateral before retrieval:
      //     const active_col_BeforeERC20 = await activePool.getAssetBalance(
      //       erc20.address
      //     );
      //     const stability_col_BeforeERC20 = await stakedDebtToken.getCollateral(
      //       erc20.address
      //     );

      //     // Expect alice to be entitled to 15000/200000 of the liquidated coll

      //     const aliceExpectedGainERC20 = liquidatedCollERC20
      //       .mul(toBN(dec(15_000, 18)))
      //       .div(toBN(dec(200_000, 18)));
      //     const idx = validCollateral.indexOf(erc20.address);
      //     const aliceGainERC20 = (
      //       await stakedDebtToken.getDepositorGains(alice, validCollateral)
      //     )[1][idx];
      ...
      //     const active_col_AfterERC20 = await activePool.getAssetBalance(
      //       erc20.address
      //     );
      //     const stability_col_AfterERC20 = await stakedDebtToken.getCollateral(
      //       erc20.address
      //     );

      //     const active_col_DifferenceERC20 = active_col_BeforeERC20.sub(
      //       active_col_AfterERC20
      //     );
      //     const stability_col_DifferenceERC20 = stability_col_BeforeERC20.sub(
      //       stability_col_AfterERC20
      //     );

      //     assert.equal(active_col_DifferenceERC20, "0");

      //     // Expect StakedDebtToken to have decreased by Alice's AssetGain
      //     assert.isAtMost(
      //       th.getDifference(stability_col_DifferenceERC20, aliceGainERC20),
      //       10000
      //     );
      //   });

      //   it("withdraw(): withdraw from SP forfeiting gains", async () => {
      //     await openWhaleVessel(
      //       erc20,
      //       (icr = 10),
      //       (extraDebtTokenAmt = 1_000_000)
      ...
      //     await stakedDebtToken.withdrawFromSP(dec(1000, 18), [], { from: alice });
      ...
      //     const liquidationTx_2ERC20 = await vesselManagerOperations.liquidate(
      //       erc20.address,
      //       defaulter_2,
      //       {
      //         from: owner,
      //       }
      //     ); // 180 GRAI closed
      //     await stakedDebtToken.withdrawFromSP(dec(199_000, 18), validCollateral, {
      //       from: whale,
      //     });
      //     const stability_col_AfterWhaleERC20 = await stakedDebtToken.getCollateral(
      //       erc20.address
      //     );
      //     assert.isAtMost(
      //       stability_col_AfterWhaleERC20.sub(aliceExpectedGainERC20),
      //       10000
      //     );
      //   });

      //   it("withdraw(): withdraw from SP, passing same asset twice will revert", async () => {
      //     await openWhaleVessel(
      //       erc20,
      //       (icr = 10),
      //       (extraDebtTokenAmt = 1_000_000)
      ...
      //       from: whale,
      //     });
      //     const txPromise = stakedDebtToken.withdrawFromSP(
      //       dec(1000, 18),
      //       [erc20.address, erc20.address],
      //       { from: whale }
      //     );
      //     await assertRevert(txPromise, "StakedDebtToken__DuplicateElementOnArray");
      //   });

      //   it("withdraw(): All depositors are able to withdraw from the SP to their account", async () => {
      //     await openWhaleVessel(erc20, (icr = 10));
      ...
      //       from: alice,
      //     });
      //     assert.equal((await stakedDebtToken.deposit(alice)).toString(), "0");
      //     await stakedDebtToken.withdrawFromSP(dec(10000, 18), validCollateral, {
      //       from: bob,
      //     });
      //     assert.equal((await stakedDebtToken.deposit(alice)).toString(), "0");
      ...
      //   it("withdraw(): increases depositor's GRAI token balance by the expected amount", async () => {
      //     await openWhaleVessel(erc20, (icr = 10));
      ...
      //       await vesselManager.getEntireDebtAndColl(erc20.address, defaulter_1)
      //     )[0];
      ...
      //         toBN("13333333333333333333333")
      //       ),
      //       100000
      //     );
      //   });

      //   it("withdraw(): doesn't impact other users Stability deposits or collateral gains", async () => {
      //     await openWhaleVessel(erc20, (icr = 10));
      ...
      //     await stakedDebtToken.provideToSP(dec(10_000, 18), validCollateral, {
      //       from: alice,
      //     });
      //     await stakedDebtToken.provideToSP(dec(20_000, 18), validCollateral, {
      //       from: bob,
      //     });
      //     await stakedDebtToken.provideToSP(dec(30_000, 18), validCollateral, {
      //       from: carol,
      //     });
      ...
      //     const debtInPool = await stakedDebtToken.getTotalDebtTokenDeposits();
      //     const collInPool = await stakedDebtToken.getCollateral(erc20.address);
      ...
      //     assert.equal(
      //       (await stakedDebtToken.deposit(carol)).toString(),
      //       dec(30_000, 18)
      //     );
      //     await stakedDebtToken.withdrawFromSP(dec(30_000, 18), validCollateral, {
      //       from: carol,
      //     });
      //     assert.equal((await stakedDebtToken.deposit(carol)).toString(), "0");
      ...
      //   it("withdraw(): doesn't impact system debt, collateral or TCR ", async () => {
      //     await openWhaleVessel(erc20, (icr = 10));
      ...
      //       dec(30000, 18)
      //     );
      //     await stakedDebtToken.withdrawFromSP(dec(30000, 18), validCollateral, {
      //       from: carol,
      //     });
      //     assert.equal((await stakedDebtToken.deposit(carol)).toString(), "0");
      ...
      //   it("withdraw(): doesn't impact any vessels, including the caller's vessel", async () => {
      //     await openWhaleVessel(erc20, (icr = 10));
      ...
      //   it("withdraw(): succeeds when amount is 0 and system has an undercollateralized vessel", async () => {
      //     await _openVessel(erc20, 100, alice);
      //     await stakedDebtToken.provideToSP(dec(100, 18), validCollateral, {
      //       from: alice,
      //     });

      //     const A_initialDeposit = (
      //       await stakedDebtToken.deposit(alice)
      //     ).toString();
      ...
      //     const idx = validCollateral.indexOf(erc20.address);
      //     const A_pendingColGain = (
      //       await stakedDebtToken.getDepositorGains(alice, validCollateral)
      //     )[1][idx];
      //     const A_pendingGRVTGain = await stakedDebtToken.getDepositorGRVTGain(
      //       alice
      //     );
      ...
      //     const tx = await stakedDebtToken.withdrawFromSP(0, validCollateral, {
      //       from: alice,
      //     });
      ...
      //     assert.isTrue(
      //       A_BalAfterERC20.sub(A_BalBeforeERC20).eq(A_pendingColGain)
      //     );
      //     assert.isTrue(A_GRVTBalDiff.sub(A_pendingGRVTGain).eq(toBN("0")));
      //   });

      //   it("withdraw(): withdrawing 0 GRAI doesn't alter the caller's deposit or the total GRAI in the Stability Pool", async () => {
      //     // --- SETUP ---
      //     await openWhaleVessel(erc20, (icr = 10));
      ...
      //     await stakedDebtToken.withdrawFromSP(0, validCollateral, { from: bob });

      //     // check Bob's deposit and total GRAI in Stability Pool has not changed
      //     const bob_Deposit_After = (
      //       await stakedDebtToken.getCompoundedDebtTokenDeposits(bob)
      //     ).toString();
      //     const GRAIinSP_After = (
      //       await stakedDebtToken.getTotalDebtTokenDeposits()
      //     ).toString();
      ...
      //   it("withdraw(): withdrawing 0 Collateral Gain does not alter the caller's Collateral balance, their vessel collateral, or the collateral in the Stability Pool", async () => {
      //     await openWhaleVessel(erc20, (icr = 10));
      ...
      //     const dennis_Balance_BeforeERC20 = (
      //       await erc20.balanceOf(dennis)
      //     ).toString();
      //     const dennis_Collateral_BeforeERC20 = (
      //       await vesselManager.Vessels(dennis, erc20.address)
      //     )[th.VESSEL_COLL_INDEX].toString();
      //     const ColinSP_BeforeERC20 = (
      //       await stakedDebtToken.getCollateral(erc20.address)
      //     ).toString();
      ...
      //     await stakedDebtToken.withdrawFromSP(dec(100, 18), validCollateral, {
      //       from: dennis,
      //     });

      //     // Check withdrawal does not alter Dennis' collateral balance or his vessel's collateral
      //     const dennis_Balance_AfterERC20 = (
      //       await erc20.balanceOf(dennis)
      //     ).toString();
      //     const dennis_Collateral_AfterERC20 = (
      //       await vesselManager.Vessels(dennis, erc20.address)
      //     )[th.VESSEL_COLL_INDEX].toString();
      //     const ColinSP_AfterERC20 = (
      //       await stakedDebtToken.getCollateral(erc20.address)
      //     ).toString();

      //     assert.equal(dennis_Balance_BeforeERC20, dennis_Balance_AfterERC20);
      //     assert.equal(
      //       dennis_Collateral_BeforeERC20,
      //       dennis_Collateral_AfterERC20
      //     );
      ...
      //   it("withdraw(): Request to withdraw > caller's deposit only withdraws the caller's compounded deposit", async () => {
      //     // --- SETUP ---
      //     await openWhaleVessel(erc20, (icr = 10));
      ...
      //       from: alice,
      //     });
      //     await stakedDebtToken.provideToSP(dec(20000, 18), validCollateral, {
      //       from: bob,
      //     });
      //     await stakedDebtToken.provideToSP(dec(30000, 18), validCollateral, {
      //       from: carol,
      //     });
      ...
      //     const GRAIinSP_Before = await stakedDebtToken.getTotalDebtTokenDeposits();

      //     await priceFeed.setPrice(erc20.address, dec(200, 18));

      //     // Bob attempts to withdraws 1 wei more than his compounded deposit from the Stability Pool
      //     await stakedDebtToken.withdrawFromSP(
      ...
      //     const bob_expectedGRAIBalance = bob_GRAI_Balance_Before
      //       .add(bob_Deposit_Before)
      //       .toString();
      //     const bob_GRAI_Balance_After = (
      //       await debtToken.balanceOf(bob)
      //     ).toString();
      ...
      //     const alice_expectedGRAIBalance = alice_GRAI_Balance_Before
      //       .add(alice_Deposit_Before)
      //       .toString();
      //     const alice_GRAI_Balance_After = (
      //       await debtToken.balanceOf(alice)
      //     ).toString();
      ...
      //   it("withdraw(): Request to withdraw 2^256-1 GRAI only withdraws the caller's compounded deposit", async () => {
      //     // --- SETUP ---
      //     await openWhaleVessel(erc20, (icr = 10));
      ...
      //     await stakedDebtToken.withdrawFromSP(maxBytes32, validCollateral, {
      //       from: bob,
      //     });
      ...
      //     const expectedGRAIinSP =
      //       GRAIinSP_Before.sub(bob_Deposit_Before).toString();
      //     const GRAIinSP_After = (
      //       await stakedDebtToken.getTotalDebtTokenDeposits()
      //     ).toString();
      //     assert.equal(GRAIinSP_After, expectedGRAIinSP);
      ...
      //   it("withdraw(): caller can withdraw full deposit and collateral gain during Recovery Mode", async () => {
      //     // Price doubles
      ...
      //       from: alice,
      //     });
      //     await stakedDebtToken.provideToSP(dec(5000, 18), validCollateral, {
      //       from: bob,
      //     });
      //     await stakedDebtToken.provideToSP(dec(3000, 18), validCollateral, {
      //       from: carol,
      //     });
      ...
      //     const alice_Balance_BeforeERC20 = web3.utils.toBN(
      //       await erc20.balanceOf(alice)
      //     );
      //     const bob_Balance_BeforeERC20 = web3.utils.toBN(
      //       await erc20.balanceOf(bob)
      //     );
      //     const carol_Balance_BeforeERC20 = web3.utils.toBN(
      ...
      //       from: alice,
      //     });
      //     await stakedDebtToken.withdrawFromSP(dec(5000, 18), validCollateral, {
      //       from: bob,
      //     });
      //     await stakedDebtToken.withdrawFromSP(dec(3000, 18), validCollateral, {
      //       from: carol,
      //     });
      ...
      //       .add(bob_Deposit_Before)
      //       .toString();
      //     const carol_expectedGRAIBalance = carol_GRAI_Balance_Before
      ...
      //       await debtToken.balanceOf(bob)
      //     ).toString();
      //     const carol_GRAI_Balance_After = (
      ...
      //     const alice_expectedColBalance = alice_Balance_BeforeERC20
      //       .add(alice_Gain_Before)
      //       .toString();
      //     const bob_expectedETHBalance = bob_Balance_BeforeERC20
      //       .add(bob_Gain_Before)
      //       .toString();
      //     const carol_expectedETHBalance = carol_Balance_BeforeERC20
      //       .add(carol_Gain_Before)
      //       .toString();

      //     const alice_Balance_AfterERC20 = (
      //       await erc20.balanceOf(alice)
      //     ).toString();
      //     const bob_Balance_AfterERC20 = (await erc20.balanceOf(bob)).toString();
      //     const carol_Balance_AfterERC20 = (
      //       await erc20.balanceOf(carol)
      //     ).toString();
      ...
      //   it("withdraw(): triggers GRVT reward event - increases the sum G", async () => {
      //     await openWhaleVessel(
      //       erc20,
      //       (icr = 10),
      //       (extraDebtTokenAmt = 1_000_000)
      ...
      //     // A withdraws from SP
      //     await stakedDebtToken.withdrawFromSP(dec(5_000, 18), validCollateral, {
      //       from: alice,
      //     });
      //     const G_1 = await stakedDebtToken.epochToScaleToG(0, 0);
      ...
      //       from: bob,
      //     });
      //     const G_2 = await stakedDebtToken.epochToScaleToG(0, 0);

      //     // Expect G has increased from the GRVT reward event triggered
      //     assert.isTrue(G_2.gt(G_1));
      //   });

      //   it("withdraw(): partial withdrawal = depositor receives GRVT rewards", async () => {
      //     await openWhaleVessel(erc20);
      ...
      //       from: carol,
      //     });
      //     await stakedDebtToken.provideToSP(dec(40, 18), validCollateral, {
      //       from: dennis,
      //     });
      ...
      //     // A, B, C withdraw
      //     await stakedDebtToken.withdrawFromSP(dec(1, 18), validCollateral, {
      //       from: alice,
      //     });
      //     await stakedDebtToken.withdrawFromSP(dec(2, 18), validCollateral, {
      //       from: bob,
      //     });
      //     await stakedDebtToken.withdrawFromSP(dec(3, 18), validCollateral, {
      //       from: carol,
      //     });
      //     await stakedDebtToken.withdrawFromSP(dec(4, 18), validCollateral, {
      //       from: dennis,
      //     });
      ...
      //   it("withdraw(): full withdrawal = zero's depositor's snapshots", async () => {
      //     await openWhaleVessel(erc20);
      //     await openWhaleVessel(erc20B);
      ...
      //     );
      //     assert.isFalse(
      //       await th.checkRecoveryMode(contracts.core, erc20B.address)
      //     );

      //     await vesselManagerOperations.liquidate(erc20.address, defaulter_1);
      //     await vesselManagerOperations.liquidate(erc20B.address, defaulter_2);

      //     const currentEpoch = await stakedDebtToken.currentEpoch();
      //     const currentScale = await stakedDebtToken.currentScale();

      //     const S_Before_erc20A = await stakedDebtToken.epochToScaleToSum(
      //       erc20.address,
      //       currentEpoch,
      //       currentScale
      //     );
      //     const S_Before_erc20B = await stakedDebtToken.epochToScaleToSum(
      //       erc20B.address,
      //       currentEpoch,
      //       currentScale
      //     );
      ...
      //       from: carol,
      //     });
      //     await stakedDebtToken.provideToSP(dec(40_000, 18), validCollateral, {
      //       from: dennis,
      //     });
      //     await stakedDebtToken.provideToSP(dec(30_000, 18), validCollateral, {
      //       from: erin,
      ...
      //       const S = await stakedDebtToken.S(depositor, erc20.address);
      //       assert.isTrue(S.eq(S_Before_erc20A));
      //       // Check S,P, G snapshots are non-zero
      //       const snapshot = await stakedDebtToken.depositSnapshots(depositor);
      ...
      //       const S = await stakedDebtToken.S(depositor, erc20B.address);
      ...
      //     await stakedDebtToken.withdrawFromSP(dec(10_000, 18), validCollateral, {
      //       from: alice,
      //     });
      //     await stakedDebtToken.withdrawFromSP(dec(20_000, 18), validCollateral, {
      //       from: bob,
      //     });
      //     await stakedDebtToken.withdrawFromSP(dec(30_000, 18), validCollateral, {
      //       from: carol,
      //     });
      //     await stakedDebtToken.withdrawFromSP(dec(40_000, 18), validCollateral, {
      //       from: dennis,
      //     });
      //     await stakedDebtToken.withdrawFromSP(dec(30_000, 18), validCollateral, {
      //       from: erin,
      ...
      //       const S = await stakedDebtToken.S(depositor, erc20Address);
      //       assert.equal(S, "0");
      //       const snapshot = await stakedDebtToken.depositSnapshots(depositor);
      ...
      //   it("withdraw(): reverts when initial deposit value is 0", async () => {
      //     await openWhaleVessel(erc20, (icr = 10));
      ...
      //     await stakedDebtToken.withdrawFromSP(dec(10_100, 18), validCollateral, {
      //       from: alice,
      //     });
      //     assert.equal(await stakedDebtToken.deposit(alice), "0");

      //     const expectedRevertMessage =
      //       "StakedDebtToken: User must have a non-zero deposit";

      //     // Further withdrawal attempt from Alice
      //     await th.assertRevert(
      //       stakedDebtToken.withdrawFromSP(dec(10_000, 18), validCollateral, {
      //         from: alice,
      //       }),
    ...
    // 		await stakedDebtToken.provideToSP(dec(10_000, 18), validCollateral, { from: alice })
    // 		await stakedDebtToken.provideToSP(dec(5_000, 18), validCollateral, { from: bob })
    ...
    // 		const poolBalance = (await stakedDebtToken.getTotalDebtTokenDeposits()).toString()
    // 		assert.equal(poolBalance, "0")

    // 		// Check Stability deposits have been fully cancelled with debt, and are now all zero
    // 		const alice_Deposit = (await stakedDebtToken.getCompoundedDebtTokenDeposits(alice)).toString()
    // 		const bob_Deposit = (await stakedDebtToken.getCompoundedDebtTokenDeposits(bob)).toString()
    ...
    // 		const idx = validCollateral.indexOf(erc20.address)
    // 		const alice_Gain_1 = (await stakedDebtToken.getDepositorGains(alice, validCollateral))[1][idx].toString()
    // 		const bob_Gain_1 = (await stakedDebtToken.getDepositorGains(bob, validCollateral))[1][idx].toString()

    // 		// Whale deposits 10_000 tokens to Stability Pool
    // 		await stakedDebtToken.provideToSP(dec(1, 24), validCollateral, { from: whale })
    ...
    // 		const alice_Gain_2 = (await stakedDebtToken.getDepositorGains(alice, validCollateral))[1][idx].toString()
    // 		const bob_Gain_2 = (await stakedDebtToken.getDepositorGains(bob, validCollateral))[1][idx].toString()
    ...
    // 		const alice_Gain_3 = (await stakedDebtToken.getDepositorGains(alice, validCollateral))[1][idx].toString()
    // 		const bob_Gain_3 = (await stakedDebtToken.getDepositorGains(bob, validCollateral))[1][idx].toString()
