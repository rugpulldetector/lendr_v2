...
const deploy = async (treasury, mintingAccounts) => {
  contracts = await deploymentHelper.deployTestContracts(
    treasury,
    mintingAccounts
  );
  ...
  stakedDebtToken = contracts.core.stakedDebtToken;
  vesselManager = contracts.core.vesselManager;
  vesselManagerOperations = contracts.core.vesselManagerOperations;
  shortTimelock = contracts.core.shortTimelock;
  longTimelock = contracts.core.longTimelock;

  lndrStaking = contracts.lndr.lndrStaking;
  lndrToken = contracts.lndr.lndrToken;
  communityIssuance = contracts.lndr.communityIssuance;
  validCollateral = await adminContract.getValidCollateral();

  // getDepositorGains() expects a sorted collateral array
  validCollateral = validCollateral
    .slice(0)
    .sort((a, b) => toBN(a.toLowerCase()).sub(toBN(b.toLowerCase())));
};
...
contract("VesselManager", async (accounts) => {
  ...
  const getOpenVesselDebtTokenAmount = async (totalDebt, asset) =>
    th.getOpenVesselDebtTokenAmount(contracts.core, totalDebt, asset);
  ...
  const withdrawDebtToken = async (params) =>
    th.withdrawDebtToken(contracts.core, params);
  const calcSoftnedAmount = (collAmount, price) =>
    collAmount
      .mul(mv._1e18BN)
      .mul(REDEMPTION_SOFTENING_PARAM)
      .div(toBN(10000))
      .div(price);
  ...
  describe("Vessel Manager", async () => {
    before(async () => {
      ...
      await lndrToken.unprotectedMint(multisig, dec(1, 24));
      ...
      await vesselManagerOperations.setRedemptionSofteningParam("9700");
//      await stopImpersonatingAccount(shortTimelock.address);
      REDEMPTION_SOFTENING_PARAM =
        await vesselManagerOperations.redemptionSofteningParam();

      console.log("REDEMPTION_SOFTENING_PARAM", REDEMPTION_SOFTENING_PARAM);
   ...
   describe("Liquidations", async () => {
      ...
      //   const ICR_Before_Asset = await vesselManager.getCurrentICR(
      //     erc20.address,
      //     alice,
      //     price
      //   );
      ...
      //   assert.equal(
      //     (await adminContract.getMcr(erc20.address)).toString(),
      //     "1100000000000000000"
      //   );
      ...
      //   await withdrawDebtToken({
      //     asset: erc20.address,
      //     ICR: targetICR,
      //     extraParams: { from: alice },
      //   });

      //   const ICR_AfterWithdrawal_Asset = await vesselManager.getCurrentICR(
      //     erc20.address,
      //     alice,
      //     price
      //   );
      //   assert.isAtMost(
      //     th.getDifference(ICR_AfterWithdrawal_Asset, targetICR),
      //     100
      //   );

      //   // price drops to 1ETH:100DebtToken, reducing Alice's ICR below MCR
      //   await priceFeed.setPrice(erc20.address, "100000000000000000000");

      //   // Confirm system is not in Recovery Mode
      //   assert.isFalse(
      //     await th.checkRecoveryMode(contracts.core, erc20.address)
      //   );
      ...
      //   const status_Asset = (
      //     await vesselManager.Vessels(alice, erc20.address)
    ...
    //   it("liquidate(): decreases ActivePool ETH and DebtTokenDebt by correct amounts", async () => {
    //     // --- SETUP ---
    //     const { collateral: A_collateral_Asset, totalDebt: A_totalDebt_Asset } =
    //       await openVessel({
    //         asset: erc20.address,
    //         ICR: toBN(dec(4, 18)),
    //         extraParams: { from: alice },
    //       });
    //     const { collateral: B_collateral_Asset, totalDebt: B_totalDebt_Asset } =
    //       await openVessel({
    //         asset: erc20.address,
    //         ICR: toBN(dec(21, 17)),
    ...
    //     // check ActivePool ETH and DebtToken debt before
    //     const activePool_ETH_Before_Asset = (
    //       await activePool.getAssetBalance(erc20.address)
    //     ).toString();
    //     const activePool_RawEther_Before_Asset = (
    //       await erc20.balanceOf(activePool.address)
    //     ).toString();
    //     const activePooL_DebtTokenDebt_Before_Asset = (
    //       await activePool.getDebtTokenBalance(erc20.address)
    //     ).toString();

    //     const stakedDebtToken_ETH_Before_Asset = (
    //       await erc20.balanceOf(stakedDebtToken.address)
    //     ).toString();

    //     const deftaultPool_ETH_Before_Asset = (
    //       await erc20.balanceOf(defaultPool.address)
    //     ).toString();

    //     assert.equal(
    //       activePool_ETH_Before_Asset,
    ...
    //       activePooL_DebtTokenDebt_Before_Asset,
    //       A_totalDebt_Asset.add(B_totalDebt_Asset)
    //     );

    //     // price drops to 1ETH:100DebtToken, reducing Bob's ICR below MCR
    //     await priceFeed.setPrice(erc20.address, "100000000000000000000");

    //     console.log("activePool_RawEther_Before_Asset", ethers.utils.formatUnits(activePool_RawEther_Before_Asset.toString(),18));
    //     console.log("stakedDebtToken_ETH_Before_Asset", ethers.utils.formatUnits(stakedDebtToken_ETH_Before_Asset.toString(),18));
    //     console.log("deftaultPool_ETH_Before_Asset", ethers.utils.formatUnits(deftaultPool_ETH_Before_Asset.toString(),18));
    ...
    //     /* close Bob's Vessel. Should liquidate his ether and DebtToken,
    // leaving Alice’s ether and DebtToken debt in the ActivePool. */
    //     await vesselManagerOperations.liquidate(erc20.address, bob, {
    //       from: owner,
    //     });

    //     // check ActivePool ETH and DebtToken debt
    //     const activePool_ETH_After_Asset = (
    //       await activePool.getAssetBalance(erc20.address)
    //     ).toString();
    //     const activePool_RawEther_After_Asset = (
    //       await erc20.balanceOf(activePool.address)
    //     ).toString();
    //     const activePooL_DebtTokenDebt_After_Asset = (
    //       await activePool.getDebtTokenBalance(erc20.address)
    //     ).toString();

    //     const stakedDebtToken_ETH_After_Asset = (
    //       await erc20.balanceOf(stakedDebtToken.address)
    //     ).toString();

    //     const defaultPool_ETH_After_Asset = (
    //       await erc20.balanceOf(defaultPool.address)
    //     ).toString();

    //     console.log("\nactivePool_RawEther_After_Asset", ethers.utils.formatUnits(activePool_RawEther_After_Asset.toString(),18));
    //     console.log("stakedDebtToken_ETH_After_Asset", ethers.utils.formatUnits(stakedDebtToken_ETH_After_Asset.toString(),18));
    //     console.log("defaultPool_ETH_After_Asset", ethers.utils.formatUnits(defaultPool_ETH_After_Asset.toString(),18));
    ...
    //     th.assertIsApproximatelyEqual(
    //       activePooL_DebtTokenDebt_After_Asset,
    //       A_totalDebt_Asset
    //     );
    //   });

  //     it("liquidate(): increases DefaultPool ETH and DebtToken debt by correct amounts", async () => {
  //       // --- SETUP ---
  //       await openVessel({
  //         asset: erc20.address,
  //         ICR: toBN(dec(4, 18)),
  ...
  //       // check DefaultPool ETH and DebtToken debt before
  //       const defaultPool_ETH_Before_Asset = await defaultPool.getAssetBalance(
  //         erc20.address
  //       );
  //       const defaultPool_RawEther_Before_Asset = (
  //         await web3.eth.getBalance(defaultPool.address)
  //       ).toString();
  //       const defaultPooL_DebtTokenDebt_Before_Asset = (
  ...
  //       assert.equal(defaultPooL_DebtTokenDebt_Before_Asset, "0");
  ...
  //       ).toString();
  //       const defaultPool_RawEther_After_Asset = (
  //         await erc20.balanceOf(defaultPool.address)
  //       ).toString();
  //       const defaultPooL_DebtTokenDebt_After_Asset = (
  //         await defaultPool.getDebtTokenBalance(erc20.address)
  //       ).toString();
  //       const defaultPool_ETH_Asset =
  //         th.applyLiquidationFee(B_collateral_Asset);
  ...
  //       th.assertIsApproximatelyEqual(
  //         defaultPooL_DebtTokenDebt_After_Asset,
  //         B_totalDebt_Asset
  //       );
  //     });
  ...
  //       const totalStakes_Before_Asset = (
  //         await vesselManager.totalStakes(erc20.address)
  //       ).toString();
  //       assert.equal(
  //         totalStakes_Before_Asset,
  //         A_collateral_Asset.add(B_collateral_Asset)
  //       );
  ...
  //       const arrayLength_Before_Asset =
  //         await vesselManager.getVesselOwnersCount(erc20.address);
  ...
  //       const vessel_0_Asset = await vesselManager.VesselOwners(
  //         erc20.address,
  //         0
  //       );
  //       const vessel_1_Asset = await vesselManager.VesselOwners(
  //         erc20.address,
  //         1
  //       );
  //       const vessel_2_Asset = await vesselManager.VesselOwners(
  //         erc20.address,
  //         2
  //       );
  //       const vessel_3_Asset = await vesselManager.VesselOwners(
  //         erc20.address,
  //         3
  //       );
  //       const vessel_4_Asset = await vesselManager.VesselOwners(
  //         erc20.address,
  //         4
  //       );
  ...
  //       const whale_arrayIndex_Asset = (
  //         await vesselManager.Vessels(whale, erc20.address)
  //       )[th.VESSEL_ARRAY_INDEX];
  //       const alice_arrayIndex_Asset = (
  //         await vesselManager.Vessels(alice, erc20.address)
  //       )[th.VESSEL_ARRAY_INDEX];
  //       const bob_arrayIndex_Asset = (
  //         await vesselManager.Vessels(bob, erc20.address)
  //       )[th.VESSEL_ARRAY_INDEX];
  //       const dennis_arrayIndex_Asset = (
  //         await vesselManager.Vessels(dennis, erc20.address)
  //       )[th.VESSEL_ARRAY_INDEX];
  //       const erin_arrayIndex_Asset = (
  //         await vesselManager.Vessels(erin, erc20.address)
  //       )[th.VESSEL_ARRAY_INDEX];
  ...
  //       // close Bob's Vessel.  His ether*0.995 and DebtToken should be added to the DefaultPool.
  //       await vesselManagerOperations.liquidate(erc20.address, bob, {
  //         from: owner,
  //       });
  ...
  //     it("liquidate(): updates the L_ETH and L_DebtTokenDebt reward-per-unit-staked totals", async () => {
  //       // --- SETUP ---
  //       const { collateral: A_collateral_Asset, totalDebt: A_totalDebt_Asset } =
  //         await openVessel({
  //           asset: erc20.address,
  ...
  //           extraParams: { from: bob },
  //         });
  //       const { collateral: C_collateral_Asset, totalDebt: C_totalDebt_Asset } =
  //         await openVessel({
  //           asset: erc20.address,
  //           ICR: toBN(dec(111, 16)),
  //           extraParams: { from: carol },
  ...
  //       // price drops to 1ETH:100DebtToken, reducing Carols's ICR below MCR
  //       await priceFeed.setPrice(erc20.address, "100000000000000000000");
  ...
  //       const L_ETH_AfterCarolLiquidated_Asset = await vesselManager.L_Colls(
  //         erc20.address
  //       );
  //       const L_DebtTokenDebt_AfterCarolLiquidated_Asset =
  //         await vesselManager.L_Debts(erc20.address);

  //       const L_ETH_expected_1_Asset = th
  //         .applyLiquidationFee(C_collateral_Asset)
  //         .mul(mv._1e18BN)
  //         .div(A_collateral_Asset.add(B_collateral_Asset));
  //       const L_DebtTokenDebt_expected_1_Asset = C_totalDebt_Asset.mul(
  //         mv._1e18BN
  //       ).div(A_collateral_Asset.add(B_collateral_Asset));
  //       assert.isAtMost(
  //         th.getDifference(
  //           L_ETH_AfterCarolLiquidated_Asset,
  //           L_ETH_expected_1_Asset
  //         ),
  //         100
  //       );
  //       assert.isAtMost(
  //         th.getDifference(
  //           L_DebtTokenDebt_AfterCarolLiquidated_Asset,
  //           L_DebtTokenDebt_expected_1_Asset
  //         ),
  //         100
  //       );

  //       // Bob now withdraws DebtToken, bringing his ICR to 1.11
  //       const { increasedTotalDebt: B_increasedTotalDebt_Asset } =
  //         await withdrawDebtToken({
  //           asset: erc20.address,
  //           ICR: toBN(dec(111, 16)),
  //           extraParams: { from: bob },
  ...
  //       // price drops to 1ETH:50DebtToken, reducing Bob's ICR below MCR
  ...
  //       const L_ETH_AfterBobLiquidated_Asset = await vesselManager.L_Colls(
  //         erc20.address
  //       );
  //       const L_DebtTokenDebt_AfterBobLiquidated_Asset =
  //         await vesselManager.L_Debts(erc20.address);
  ...
  //  L_DebtTokenDebt = (180 / 20) + (890 / 10) = 98 DebtToken */

  //       const L_ETH_expected_2_Asset = L_ETH_expected_1_Asset.add(
  //         th
  //           .applyLiquidationFee(
  //             B_collateral_Asset.add(
  //               B_collateral_Asset.mul(L_ETH_expected_1_Asset).div(mv._1e18BN)
  //             )
  //           )
  //           .mul(mv._1e18BN)
  //           .div(A_collateral_Asset)
  //       );
  //       const L_DebtTokenDebt_expected_2 = L_DebtTokenDebt_expected_1_Asset.add(
  //         B_totalDebt_Asset.add(B_increasedTotalDebt_Asset)
  //           .add(
  //             B_collateral_Asset.mul(L_DebtTokenDebt_expected_1_Asset).div(
  //               mv._1e18BN
  //             )
  //           )
  //           .mul(mv._1e18BN)
  ...
  //       assert.isAtMost(
  //         th.getDifference(
  //           L_ETH_AfterBobLiquidated_Asset,
  //           L_ETH_expected_2_Asset
  //         ),
  //         100
  //       );
  //       assert.isAtMost(
  //         th.getDifference(
  //           L_DebtTokenDebt_AfterBobLiquidated_Asset,
  //           L_DebtTokenDebt_expected_2
  //         ),
  //         100
  //       );
  //     });
  ...
  //       // Alice creates a single vessel with 0.7 ETH and a debt of 70 DebtToken, and provides 10 DebtToken to SP
  //       await openVessel({
  //         asset: erc20.address,
  //         ICR: toBN(dec(2, 18)),
  //         extraParams: { from: alice },
  ...
  //       await debtToken.approve(stakedDebtToken.address, dec(10, 18), {
  //         from: alice,
  //       });
  //       await stakedDebtToken.deposit(dec(10, 18), alice, {
  //         from: alice,
  //       });
  ...
  //       const alice_ICR_Asset = (
  //         await vesselManager.getCurrentICR(erc20.address, alice, price)
  //       ).toString();
  //       assert.equal(alice_ICR_Asset, "1050000000000000000");

  //       const activeVesselsCount_Before_Asset =
  //         await vesselManager.getVesselOwnersCount(erc20.address);
  ...
  //       const activeVesselsCount_After_Asset =
  //         await vesselManager.getVesselOwnersCount(erc20.address);
  //       assert.equal(activeVesselsCount_After_Asset, 1);

  //       const alice_isInSortedList_Asset = await sortedVessels.contains(
  //         erc20.address,
  //         alice
  //       );
  //       assert.isFalse(alice_isInSortedList_Asset);

  //       const bob_isInSortedList_Asset = await sortedVessels.contains(
  //         erc20.address,
  //         bob
  //       );
  ...
  //       assert.equal(
  //         await vesselManager.getVesselStatus(erc20.address, carol),
  //         0
  //       ); // check vessel non-existent
  ...
  //       try {
  //         const txCarol = await vesselManagerOperations.liquidate(
  //           erc20.address,
  //           carol
  //         );
  ...
  //         assert.include(
  //           err.message,
  //           "VesselManagerOperations__VesselNotActive()"
  //         );
  //       }
  //     });
  ...
  //       const txCarol_L1_Asset = await vesselManagerOperations.liquidate(
  //         erc20.address,
  //         carol
  //       );
  ...
  //         await vesselManager.getVesselStatus(erc20.address, carol),
  //         3
  //       ); // check vessel closed by liquidation
  ...
  //       try {
  //         const txCarol_L2 = await vesselManagerOperations.liquidate(
  //           erc20.address,
  //           carol
  //         );
  ...
  //       const TCR_Before_Asset = (
  //         await th.getTCR(contracts.core, erc20.address)
  //       ).toString();
  //       const listSize_Before_Asset = (
  //         await sortedVessels.getSize(erc20.address)
  //       ).toString();

  //       const price = await priceFeed.getPrice(erc20.address);

  //       // Check Bob's ICR > 110%
  //       const bob_ICR_Asset = await vesselManager.getCurrentICR(
  //         erc20.address,
  //         bob,
  //         price
  //       );
  ...
  //       await assertRevert(
  //         vesselManagerOperations.liquidate(erc20.address, bob),
  ...
  //       const TCR_After_Asset = (
  //         await th.getTCR(contracts.core, erc20.address)
  //       ).toString();
  //       const listSize_After_Asset = (
  //         await sortedVessels.getSize(erc20.address)
  //       ).toString();
  ...
  //       await debtToken.approve(stakedDebtToken.address, spDeposit, {
  //         from: whale,
  //       });

  //       await stakedDebtToken.deposit(spDeposit, whale, {
  //         from: whale,
  //       });
  ...
  //       assert.isFalse(
  //         await sortedVessels.contains(erc20.address, defaulter_1)
  //       );

  //       await vesselManagerOperations.liquidate(erc20.address, defaulter_2);
  //       assert.isFalse(
  //         await sortedVessels.contains(erc20.address, defaulter_2)
  //       );

  //       await vesselManagerOperations.liquidate(erc20.address, defaulter_3);
  //       assert.isFalse(
  //         await sortedVessels.contains(erc20.address, defaulter_3)
  //       );

  //       await vesselManagerOperations.liquidate(erc20.address, defaulter_4);
  //       assert.isFalse(
  //         await sortedVessels.contains(erc20.address, defaulter_4)
  //       );
  ...
  //       const entireSystemCollBefore_Asset =
  //         await vesselManager.getEntireSystemColl(erc20.address);
  //       const entireSystemDebtBefore_Asset =
  //         await vesselManager.getEntireSystemDebt(erc20.address);

  //       const expectedTCR_0_Asset = entireSystemCollBefore_Asset
  //         .mul(price)
  ...
  //       const liquidationTx_1_Asset = await vesselManagerOperations.liquidate(
  //         erc20.address,
  //         defaulter_1
  //       );

  //       const [
  //         liquidatedDebt_1_Asset,
  //         liquidatedColl_1_Asset,
  //         gasComp_1_Asset,
  ...
  //       const liquidationTx_2_Asset = await vesselManagerOperations.liquidate(
  //         erc20.address,
  //         defaulter_2
  //       );
  //       const [
  //         liquidatedDebt_2_Asset,
  //         liquidatedColl_2_Asset,
  //         gasComp_2_Asset,
  ...
  //       const liquidationTx_3_Asset = await vesselManagerOperations.liquidate(
  //         erc20.address,
  //         defaulter_3
  //       );

  //       const [
  //         liquidatedDebt_3_Asset,
  //         liquidatedColl_3_Asset,
  //         gasComp_3_Asset,
  ...
  //       const liquidationTx_4_Asset = await vesselManagerOperations.liquidate(
  //         erc20.address,
  //         defaulter_4
  //       );
  //       const [
  //         liquidatedDebt_4_Asset,
  //         liquidatedColl_4_Asset,
  //         gasComp_4_Asset,
  ...
  //         from: dennis,
  //       });
  //       await stakedDebtToken.deposit(spDeposit, dennis, {
  //         from: dennis,
  //       });
  ...
  //       const liquidationTX_C_Asset = await vesselManagerOperations.liquidate(
  //         erc20.address,
  //         carol
  //       );
  ...
  //       // const dennis_Deposit_Before_Asset = (
  //       //   await stakedDebtToken.getCompoundedDebtTokenDeposits(dennis)
  //       // ).toString();
  //       // const idx = validCollateral.indexOf(erc20.address);
  //       // const dennis_ETHGain_Before_Asset = (
  //       //   await stakedDebtToken.getDepositorGains(dennis, validCollateral)
  //       // )[1][idx].toString();
  //       // assert.isAtMost(
  //       //   th.getDifference(
  //       //     dennis_Deposit_Before_Asset,
  //       //     spDeposit.sub(liquidatedDebt_Asset)
  //       //   ),
  //       //   1000000
  //       // );
  //       // assert.isAtMost(
  //       //   th.getDifference(dennis_ETHGain_Before_Asset, liquidatedColl_Asset),
  //       //   1000
  //       // );
  ...
  //       try {
  //         const txDennis = await vesselManagerOperations.liquidate(
  //           erc20.address,
  //           dennis
  //         );
  ...
  //       // const dennis_Deposit_After_Asset = (
  //       //   await stakedDebtToken.getCompoundedDebtTokenDeposits(dennis)
  //       // ).toString();
  ...
  //         from: bob,
  //       });
  //       await stakedDebtToken.deposit(spDeposit, bob, {
  //         from: bob,
  //       });
  ...
  //       const price = await priceFeed.getPrice(erc20.address);
  //       assert.isTrue(
  //         (await vesselManager.getCurrentICR(erc20.address, bob, price)).gt(
  //           mv._MCR
  //         )
  //       );
  ...
  //       // const bob_Deposit_Before_Asset = (
  //       //   await stakedDebtToken.getCompoundedDebtTokenDeposits(bob)
  //       // ).toString();
  //       // const idx = validCollateral.indexOf(erc20.address);
  //       // const bob_ETHGain_Before_Asset = (
  //       //   await stakedDebtToken.getDepositorGains(bob, validCollateral)
  //       // )[1][idx].toString();

  //       // assert.isAtMost(
  //       //   th.getDifference(
  //       //     bob_Deposit_Before_Asset,
  //       //     spDeposit.sub(liquidatedDebt_Asset)
  //       //   ),
  //       //   1000000
  //       // );
  //       // assert.isAtMost(
  //       //   th.getDifference(bob_ETHGain_Before_Asset, liquidatedColl_Asset),
  //       //   1000
  //       // );
  ...
  //       // const bob_Deposit_After_Asset = (
  //       //   await stakedDebtToken.getCompoundedDebtTokenDeposits(bob)
  //       // ).toString();
  //       // const bob_ETHGain_After_Asset = (
  //       //   await stakedDebtToken.getDepositorGains(bob, validCollateral)
  //       // )[1][idx].toString();
  ...
  //       const { collateral: B_collateral_Asset, totalDebt: B_debt_Asset } =
  //         await openVessel({
  //           asset: erc20.address,
  //           ICR: toBN(dec(218, 16)),
  ...
  //           extraParams: { from: bob },
  //         });
  //       const { collateral: C_collateral_Asset, totalDebt: C_debt_Asset } =
  //         await openVessel({
  //           asset: erc20.address,
  //           ICR: toBN(dec(210, 16)),
  //           extraDebtTokenAmount: toBN(dec(100, 18)),
  ...
  //       //Bob provides DebtToken to SP
  //       //Dennis provides DebtToken to SP
  //       await debtToken.approve(stakedDebtToken.address, B_spDeposit, {
  //         from: bob,
  //       });
  //       await stakedDebtToken.deposit(B_spDeposit, dennis, {
  //         from: bob,
  //       });
  ...
  //       //     th.applyLiquidationFee(C_collateral_Asset)
  //       //   ),
  //       //   1000
  //       // );
  ...
  //       await debtToken.approve(stakedDebtToken.address, A_spDeposit, {
  //         from: alice,
  //       });
  //       await stakedDebtToken.deposit(A_spDeposit, alice, {
  //         from: alice,
  //       });
  ...
  //       assert.isFalse(await sortedVessels.contains(erc20.address, bob));
  //       const bob_Vessel_Status_Asset = (
  //         await vesselManager.Vessels(bob, erc20.address)
  //       )[th.VESSEL_STATUS_INDEX].toString();
  ...
  //    Check Bob' SP deposit has been reduced to 50 DebtToken, and his ETH gain has increased to 1.5 ETH. */
  //       // const alice_Deposit_After_Asset = (
  //       //   await stakedDebtToken.getCompoundedDebtTokenDeposits(alice)
  //       // ).toString();
  //       // const alice_ETHGain_After_Asset = (
  //       //   await stakedDebtToken.getDepositorGains(alice, validCollateral)
  //       // )[1][idx].toString();
  ...
  //       //     A_spDeposit.sub(
  //       //       B_debt_Asset.mul(A_spDeposit).div(totalDeposits_Asset)
  //       //     )
  //       //   ),
  //       //   1000000
  //       // );
  ...
  //       //     bob_Deposit_Before_Asset.sub(
  //       //       B_debt_Asset.mul(bob_Deposit_Before_Asset).div(
  //       //         totalDeposits_Asset
  //       //       )
  //       //     )
  //       //   ),
  ...
  //         extraParams: { from: whale },
  //       });
  //       const { debtTokenAmount: A_DebtTokenAmount_Asset } = await openVessel({
  //         asset: erc20.address,
  //         ICR: toBN(dec(2, 18)),
  //         extraDebtTokenAmount: toBN(dec(300, 18)),
  //         extraParams: { from: alice },
  //       });
  //       const { debtTokenAmount: B_DebtTokenAmount_Asset } = await openVessel({
  //         asset: erc20.address,
  //         ICR: toBN(dec(2, 18)),
  //         extraDebtTokenAmount: toBN(dec(200, 18)),
  //         extraParams: { from: bob },
  //       });
  //       const { debtTokenAmount: C_DebtTokenAmount_Asset } = await openVessel({
  //         asset: erc20.address,
  //         ICR: toBN(dec(2, 18)),
  //         extraDebtTokenAmount: toBN(dec(100, 18)),
  //         extraParams: { from: carol },
  ...
  //       assert.equal(
  //         (await sortedVessels.getSize(erc20.address)).toString(),
  //         "4"
  //       );
  ...
  //       assert.equal(
  //         (await debtToken.balanceOf(alice)).toString(),
  //         A_DebtTokenAmount_Asset.toString()
  //       );
  //       assert.equal(
  //         (await debtToken.balanceOf(bob)).toString(),
  //         B_DebtTokenAmount_Asset.toString()
  //       );
  //       assert.equal(
  //         (await debtToken.balanceOf(carol)).toString(),
  //         C_DebtTokenAmount_Asset.toString()
  //       );
  //     });
  ...
  //       // Defaulter opens with 60 DebtToken, 0.6 ETH
  //       await openVessel({
  //         asset: erc20.address,
  //         ICR: toBN(dec(2, 18)),
  //         extraParams: { from: defaulter_1 },
  ...
  //       const alice_ICR_Before_Asset = await vesselManager.getCurrentICR(
  //         erc20.address,
  //         alice,
  //         price
  //       );
  //       const bob_ICR_Before_Asset = await vesselManager.getCurrentICR(
  //         erc20.address,
  //         bob,
  //         price
  //       );
  //       const carol_ICR_Before_Asset = await vesselManager.getCurrentICR(
  //         erc20.address,
  //         carol,
  //         price
  //       );
  ...
  //       /* Liquidate defaulter. 30 DebtToken and 0.3 ETH is distributed between A, B and C.

  //   A receives (30 * 2/4) = 15 DebtToken, and (0.3*2/4) = 0.15 ETH
  //   B receives (30 * 1/4) = 7.5 DebtToken, and (0.3*1/4) = 0.075 ETH
  //   C receives (30 * 1/4) = 7.5 DebtToken, and (0.3*1/4) = 0.075 ETH
  //   */
  //       await vesselManagerOperations.liquidate(erc20.address, defaulter_1);

  //       const alice_ICR_After_Asset = await vesselManager.getCurrentICR(
  //         erc20.address,
  //         alice,
  //         price
  //       );
  //       const bob_ICR_After_Asset = await vesselManager.getCurrentICR(
  //         erc20.address,
  //         bob,
  //         price
  //       );
  //       const carol_ICR_After_Asset = await vesselManager.getCurrentICR(
  //         erc20.address,
  //         carol,
  //         price
  //       );
  ...
  //       const bob_Coll_Asset = (
  //         await vesselManager.Vessels(bob, erc20.address)
  //       )[th.VESSEL_COLL_INDEX];
  //       const bob_Debt_Asset = (
  //         await vesselManager.Vessels(bob, erc20.address)
  //       )[th.VESSEL_DEBT_INDEX];

  //       const bob_rawICR_Asset = bob_Coll_Asset
  //         .mul(toBN(dec(100, 18)))
  //         .div(bob_Debt_Asset);
  ...
  //       );
  //       assert.equal(
  //         (await vesselManager.Vessels(carol, erc20.address))[
  //           th.VESSEL_STATUS_INDEX
  //         ].toString(),
  //         "3"
  //       );
  ...
  //     it("liquidate(): when SP > 0, triggers LNDR reward event - increases the sum G", async () => {
  //       await openVessel({
  //         asset: erc20.address,
  //         ICR: toBN(dec(100, 18)),
  //         extraParams: { from: whale },
  ...
  //       // B provides to SP
  //       await debtToken.approve(stakedDebtToken.address, dec(100, 18), {
  //         from: B,
  //       });

  //       await stakedDebtToken.deposit(dec(100, 18), B, {
  //         from: B,
  //       });
  //       assert.equal(await stakedDebtToken.totalAssets(), dec(100, 18));

  //       // const G_Before_Asset = await stakedDebtToken.epochToScaleToG(0, 0);
  ...
  //       // Price drops to 1ETH:100DebtToken, reducing defaulters to below MCR
  //       await priceFeed.setPrice(erc20.address, dec(100, 18));
  //       await priceFeed.getPrice(erc20.address);
  //       assert.isFalse(
  //         await th.checkRecoveryMode(contracts.core, erc20.address)
  ...
  //       // const G_After_Asset = await stakedDebtToken.epochToScaleToG(0, 0);

  //       // Expect G has increased from the LNDR reward event triggered
  ...
  //      const earning = await stakedDebtToken.earned(B);
  //       console.log("earned: B", ethers.utils.formatUnits(earning.toString(), 18));
  //       // B withdraws
  //       await stakedDebtToken.withdraw(dec(100, 18), B, B, {
  //         from: B,
  //       });

  //       // Check SP is empty
  //       assert.equal(await stakedDebtToken.totalAssets(), "0");
  ...
  //       const liqTxA_Asset = await vesselManagerOperations.liquidate(
  //         erc20.address,
  //         A
  //       );
  ...
  //         from: A,
  //       });
  //       await stakedDebtToken.deposit(dec(10, 18), A, {
  //         from: A,
  //       });
  ...
  //       const ICR_C_Asset = await vesselManager.getCurrentICR(
  //         erc20.address,
  //         C,
  //         price
  //       );
  ...
  //       const liqTxBC_Asset = await vesselManagerOperations.liquidateVessels(
  //         erc20.address,
  //         2
  //       );
  ...
  //       assert.isTrue(
  //         (await sortedVessels.getSize(erc20.address)).eq(toBN("3"))
  //       );
  //       await borrowerOperations.repayDebtTokens(
  //         erc20.address,
  //         dec(1, 18),
  //         D,
  //         D,
  ...
  //       const pendingETH_C_Asset = await vesselManager.getPendingAssetReward(
  //         erc20.address,
  //         C
  //       );
  //       const pendingDebtTokenDebt_C_Asset =
  //         await vesselManager.getPendingDebtTokenReward(erc20.address, C);
  //       const defaultPoolETH_Asset = await defaultPool.getAssetBalance(
  //         erc20.address
  //       );
  //       const defaultPoolDebtTokenDebt_Asset =
  //         await defaultPool.getDebtTokenBalance(erc20.address);

  //       assert.isTrue(pendingETH_C_Asset.lte(defaultPoolETH_Asset));
  //       assert.isTrue(
  //         pendingDebtTokenDebt_C_Asset.lte(defaultPoolDebtTokenDebt_Asset)
  //       );
  //       //Check only difference is dust

  //       assert.isAtMost(
  //         th.getDifference(pendingETH_C_Asset, defaultPoolETH_Asset),
  //         1000
  //       );
  //       assert.isAtMost(
  //         th.getDifference(
  //           pendingDebtTokenDebt_C_Asset,
  //           defaultPoolDebtTokenDebt_Asset
  //         ),
  //         1000
  //       );
  ...
  //       await debtToken.approve(stakedDebtToken.address, dec(50, 18), {
  //         from: D,
  //       });
  //       await stakedDebtToken.deposit(dec(50, 18), D, {
  //         from: D,
  //       });
  ...
  //         from: E,
  //       });
  //       await stakedDebtToken.deposit(dec(50, 18), E, {
  //         from: E,
  //       });
  ...
  //       const liqTx2_Asset = await vesselManagerOperations.liquidateVessels(
  //         erc20.address,
  //         2
  //       );
  ...
  //       assert.isTrue(await sortedVessels.contains(erc20.address, E));
  //       assert.isTrue(
  //         (await sortedVessels.getSize(erc20.address)).eq(toBN("1"))
  //       );
  //     });
  ...
  //       await debtToken.approve(stakedDebtToken.address, dec(300, 18), {
  //         from: whale,
  //       });
  //       await stakedDebtToken.deposit(dec(300, 18), whale, {
  //         from: whale,
  //       });

  //       // --- TEST ---

  //       // Price drops to 1ETH:100DebtToken, reducing Bob and Carol's ICR below MCR
  //       await priceFeed.setPrice(erc20.address, dec(100, 18));
  //       const price = await priceFeed.getPrice(erc20.address);
  ...
  //       assert.isTrue(
  //         (await vesselManager.getCurrentICR(erc20.address, alice, price)).lte(
  //           mv._MCR
  //         )
  //       );
  //       assert.isTrue(
  //         (await vesselManager.getCurrentICR(erc20.address, bob, price)).lte(
  //           mv._MCR
  //         )
  //       );
  //       assert.isTrue(
  //         (await vesselManager.getCurrentICR(erc20.address, carol, price)).lte(
  //           mv._MCR
  //         )
  //       );
  //       assert.isTrue(
  //         (await vesselManager.getCurrentICR(erc20.address, erin, price)).lte(
  //           mv._MCR
  //         )
  //       );
  //       assert.isTrue(
  //         (await vesselManager.getCurrentICR(erc20.address, flyn, price)).lte(
  //           mv._MCR
  //         )
  //       );
  ...
  //       assert.isTrue(
  //         (await vesselManager.getCurrentICR(erc20.address, graham, price)).gte(
  //           mv._MCR
  //         )
  //       );
  //       assert.isTrue(
  //         (
  //           await vesselManager.getCurrentICR(erc20.address, harriet, price)
  //         ).gte(mv._MCR)
  //       );
  //       assert.isTrue(
  //         (await vesselManager.getCurrentICR(erc20.address, ida, price)).gte(
  //           mv._MCR
  //         )
  //       );
  ...
  //       assert.isTrue(
  //         (await vesselManager.getCurrentICR(erc20.address, whale, price)).gte(
  //           mv._MCR
  //         )
  //       );
  ...
  //       );
  //       assert.equal(
  //         (await vesselManager.Vessels(flyn, erc20.address))[
  //           th.VESSEL_STATUS_INDEX
  //         ].toString(),
  //         "3"
  //       );
  ...
  //       const VesselOwnersArrayLength_Asset =
  //         await vesselManager.getVesselOwnersCount(erc20.address);
  //       assert.equal(VesselOwnersArrayLength_Asset, "3");

  //       // Check Alice, Bob, Carol vessels have been closed

  //       const aliceVesselStatus_Asset = (
  //         await vesselManager.getVesselStatus(erc20.address, alice)
  //       ).toString();
  //       const bobVesselStatus_Asset = (
  //         await vesselManager.getVesselStatus(erc20.address, bob)
  //       ).toString();
  //       const carolVesselStatus_Asset = (
  //         await vesselManager.getVesselStatus(erc20.address, carol)
  //       ).toString();
  ...
  //         bob
  //       );
  //       const carol_isInSortedList_Asset = await sortedVessels.contains(
  //         erc20.address,
  //         carol
  //       );
  ...
  //       const dennisVesselStatus_Asset = (
  //         await vesselManager.getVesselStatus(erc20.address, dennis)
  //       ).toString();
  //       const erinVesselStatus_Asset = (
  //         await vesselManager.getVesselStatus(erc20.address, erin)
  //       ).toString();
  ...
  //       const dennis_isInSortedList_Asset = await sortedVessels.contains(
  //         erc20.address,
  //         dennis
  //       );
  //       const erin_isInSortedList_Asset = await sortedVessels.contains(
  //         erc20.address,
  //         erin
  //       );
  ...
  //       );
  //       assert.isTrue(
  //         (await vesselManager.getCurrentICR(erc20.address, alice, price)).gte(
  //           mv._MCR
  //         )
  //       );
  //       assert.isTrue(
  //         (await vesselManager.getCurrentICR(erc20.address, bob, price)).gte(
  //           mv._MCR
  //         )
  //       );
  //       assert.isTrue(
  //         (await vesselManager.getCurrentICR(erc20.address, carol, price)).gte(
  //           mv._MCR
  //         )
  //       );
  ...
  //       // Liquidate defaulter. 30 DebtToken and 0.3 ETH is distributed uniformly between A, B and C. Each receive 10 DebtToken, 0.1 ETH
  //       await vesselManagerOperations.liquidate(erc20.address, defaulter_1);
  ...
  //       const alice_ICR_Asset = await vesselManager.getCurrentICR(
  //         erc20.address,
  //         alice,
  //         price
  //       );
  ...
  //         price
  //       );
  //       const carol_ICR_Asset = await vesselManager.getCurrentICR(
  //         erc20.address,
  //         carol,
  //         price
  //       );
  ...
  //         price
  //       );
  //       const dennis_ICR_Asset = await vesselManager.getCurrentICR(
  //         erc20.address,
  //         dennis,
  //         price
  //       );
  //       const erin_ICR_Asset = await vesselManager.getCurrentICR(
  //         erc20.address,
  //         erin,
  //         price
  //       );
  //       const flyn_ICR_Asset = await vesselManager.getCurrentICR(
  //         erc20.address,
  //         flyn,
  //         price
  //       );
  ...
  //         D_balanceBefore.toString()
  //       );
  //       assert.equal(
  //         (await debtToken.balanceOf(erin)).toString(),
  //         E_balanceBefore.toString()
  //       );
  //       assert.equal(
  //         (await debtToken.balanceOf(flyn)).toString(),
  //         F_balanceBefore.toString()
  //       );
  //     });
  ...
  //       await debtToken.approve(stakedDebtToken.address, dec(500, 18), {
  //         from: whale,
  //       });
  //       await stakedDebtToken.deposit(dec(500, 18), whale, {
  //         from: whale,
  //       });
  ...
  //       assert.equal(
  //         (await stakedDebtToken.totalAssets()).toString(),
  //         dec(500, 18)
  //       );
  ...
  //       // assert.equal((await stakedDebtToken.totalAssets()).toString(), "0");
  ...
  //       const { collateral: W_coll_Asset, totalDebt: W_debt_Asset } =
  //         await openVessel({
  //           asset: erc20.address,
  //           ICR: toBN(dec(100, 18)),
  //           extraParams: { from: whale },
  //         });
  //       const { collateral: A_coll_Asset, totalDebt: A_debt_Asset } =
  //         await openVessel({
  //           asset: erc20.address,
  //           ICR: toBN(dec(4, 18)),
  //           extraParams: { from: alice },
  //         });
  //       const { collateral: B_coll_Asset, totalDebt: B_debt_Asset } =
  //         await openVessel({
  //           asset: erc20.address,
  //           ICR: toBN(dec(28, 18)),
  //           extraParams: { from: bob },
  //         });
  //       const { collateral: C_coll_Asset, totalDebt: C_debt_Asset } =
  //         await openVessel({
  //           asset: erc20.address,
  //           ICR: toBN(dec(8, 18)),
  //           extraParams: { from: carol },
  //         });
  //       const { collateral: D_coll_Asset, totalDebt: D_debt_Asset } =
  //         await openVessel({
  //           asset: erc20.address,
  //           ICR: toBN(dec(80, 18)),
  //           extraParams: { from: dennis },
  ...
  //       const { collateral: d1_coll_Asset, totalDebt: d1_debt_Asset } =
  //         await openVessel({
  //           asset: erc20.address,
  //           ICR: toBN(dec(199, 16)),
  //           extraParams: { from: defaulter_1 },
  //         });
  //       const { collateral: d2_coll_Asset, totalDebt: d2_debt_Asset } =
  //         await openVessel({
  //           asset: erc20.address,
  //           ICR: toBN(dec(156, 16)),
  //           extraParams: { from: defaulter_2 },
  //         });
  //       const { collateral: d3_coll_Asset, totalDebt: d3_debt_Asset } =
  //         await openVessel({
  //           asset: erc20.address,
  //           ICR: toBN(dec(183, 16)),
  //           extraParams: { from: defaulter_3 },
  //         });
  //       const { collateral: d4_coll_Asset, totalDebt: d4_debt_Asset } =
  //         await openVessel({
  //           asset: erc20.address,
  //           ICR: toBN(dec(166, 16)),
  //           extraParams: { from: defaulter_4 },
  ...
  //         .add(C_coll_Asset)
  //         .add(D_coll_Asset);
  //       const totalCollDefaulters_Asset = d1_coll_Asset
  //         .add(d2_coll_Asset)
  //         .add(d3_coll_Asset)
  //         .add(d4_coll_Asset);
  //       const totalColl_Asset = totalCollNonDefaulters_Asset.add(
  //         totalCollDefaulters_Asset
  //       );
  ...
  //         th.getDifference(
  //           TCR_Before_Asset,
  //           totalColl_Asset.mul(price).div(totalDebt_Asset)
  //         ),
  //         1000
  //       );
  ...
  //       assert.isTrue(
  //         TCR_After_Asset.gte(TCR_Before_Asset.mul(toBN(995)).div(toBN(1000)))
  //       );
  //     });
  ...
  //       await debtToken.approve(stakedDebtToken.address, whaleDeposit, {
  //         from: whale,
  //       });
  //       await stakedDebtToken.deposit(whaleDeposit, whale, {
  //         from: whale,
  //       });
  ...
  //       const liquidatedColl_Asset =
  //         A_coll_Asset.add(B_coll_Asset).add(C_coll_Asset);
  //       const liquidatedDebt_Asset =
  //         A_debt_Asset.add(B_debt_Asset).add(C_debt_Asset);

  //       // A, B provide 100, 300 to the SP

  //       await debtToken.approve(stakedDebtToken.address, A_deposit, {
  //         from: alice,
  //       });
  //       await stakedDebtToken.deposit(A_deposit, alice, {
  //         from: alice,
  //       });

  //       await debtToken.approve(stakedDebtToken.address, B_deposit, {
  //         from: bob,
  //       });
  //       await stakedDebtToken.deposit(B_deposit, bob, {
  //         from: bob,
  //       });
  ...
  //   whale DebtToken loss: 650 * (400/800) = 325 DebtToken
  //   alice DebtToken loss:  650 *(100/800) = 81.25 DebtToken
  //   bob DebtToken loss: 650 * (300/800) = 243.75 DebtToken

  //   whale remaining deposit: (400 - 325) = 75 DebtToken
  //   alice remaining deposit: (100 - 81.25) = 18.75 DebtToken
  //   bob remaining deposit: (300 - 243.75) = 56.25 DebtToken
  ...
  //       // Check remaining DebtToken Deposits and ETH gain, for whale and depositors whose vessels were liquidated

  //       // const whale_Deposit_After_Asset =
  //       //   await stakedDebtToken.getCompoundedDebtTokenDeposits(whale);
  //       // const alice_Deposit_After_Asset =
  //       //   await stakedDebtToken.getCompoundedDebtTokenDeposits(alice);
  //       // const bob_Deposit_After_Asset =
  //       //   await stakedDebtToken.getCompoundedDebtTokenDeposits(bob);
  ...
  //       // const idx = validCollateral.indexOf(erc20.address);
  //       // const whale_ETHGain_Asset = (
  //       //   await stakedDebtToken.getDepositorGains(whale, validCollateral)
  //       // )[1][idx];
  //       // const alice_ETHGain_Asset = (
  //       //   await stakedDebtToken.getDepositorGains(alice, validCollateral)
  //       // )[1][idx];
  //       // const bob_ETHGain_Asset = (
  //       //   await stakedDebtToken.getDepositorGains(bob, validCollateral)
  //       // )[1][idx];
  ...
  //       //     whaleDeposit.sub(
  //       //       liquidatedDebt_Asset.mul(whaleDeposit).div(totalDeposits_Asset)
  //       //     )
  //       //   ),
  //       //   100000
  //       // );
  ...
  //       //     alice_Deposit_After_Asset,
  //       //     A_deposit.sub(
  //       //       liquidatedDebt_Asset.mul(A_deposit).div(totalDeposits_Asset)
  //       //     )
  //       //   ),
  //       //   100000
  //       // );
  ...
  //       //     bob_Deposit_After_Asset,
  //       //     B_deposit.sub(
  //       //       liquidatedDebt_Asset.mul(B_deposit).div(totalDeposits_Asset)
  //       //     )
  //       //   ),
  //       //   100000
  //       // );
  ...
  //       const total_DebtTokeninSP_Asset = (
  //         await stakedDebtToken.totalAssets()
  //       ).toString();
  //       const total_ETHinSP_Asset = (
  //         await stakedDebtToken.getCollateral(erc20.address)
  //       ).toString();

  //       // assert.isAtMost(
  //       //   th.getDifference(
  //       //     total_DebtTokeninSP_Asset,
  //       //     totalDeposits_Asset.sub(liquidatedDebt_Asset)
  //       //   ),
  //       //   1000
  //       // );
  //       // assert.isAtMost(
  //       //   th.getDifference(
  //       //     total_ETHinSP_Asset,
  //       //     th.applyLiquidationFee(liquidatedColl_Asset)
  //       //   ),
  //       //   1000
  //       // );
  //     });

  //     it("liquidateVessels(): when SP > 0, triggers LNDR reward event - increases the sum G", async () => {
  //       await openVessel({
  //         asset: erc20.address,
  //         ICR: toBN(dec(100, 18)),
  //         extraParams: { from: whale },
  ...
  //       // Try to liquidate C again. Check it succeeds and closes C's vessel
  //       const liqTx2_Asset =
  //         await vesselManagerOperations.batchLiquidateVessels(erc20.address, [
  //           C,
  //           D,
  //         ]);
  //       assert.isTrue(liqTx2_Asset.receipt.status);
  ...
  //       // Price drops to 1ETH:100DebtToken, reducing A, B, C ICR below MCR
  //       await priceFeed.setPrice(erc20.address, dec(100, 18));
  //       const price = await priceFeed.getPrice(erc20.address);
  ...
  //       assert.isTrue(
  //         (await vesselManager.getCurrentICR(erc20.address, alice, price)).lt(
  //           mv._MCR
  //         )
  //       );
  //       assert.isTrue(
  //         (await vesselManager.getCurrentICR(erc20.address, bob, price)).lt(
  //           mv._MCR
  //         )
  //       );
  //       assert.isTrue(
  //         (await vesselManager.getCurrentICR(erc20.address, carol, price)).lt(
  //           mv._MCR
  //         )
  //       );
  ...
  //       assert.isTrue(
  //         (await vesselManager.getCurrentICR(erc20.address, dennis, price)).gte(
  //           mv._MCR
  //         )
  //       );
  //       assert.isTrue(
  //         (await vesselManager.getCurrentICR(erc20.address, erin, price)).gte(
  //           mv._MCR
  //         )
  //       );
  ...
  //       await vesselManagerOperations.batchLiquidateVessels(
  //         erc20.address,
  //         liquidationArray
  //       );
  ...
  //       );
  //       assert.isTrue(
  //         (await vesselManager.getCurrentICR(erc20.address, dennis, price)).lt(
  //           mv._MCR
  //         )
  //       );
  //       assert.isTrue(
  //         (await vesselManager.getCurrentICR(erc20.address, erin, price)).lt(
  //           mv._MCR
  //         )
  //       );
  ...
  //       liquidationArray = [];
  //       try {
  //         const tx = await vesselManagerOperations.batchLiquidateVessels(
  //           erc20.address,
  //           liquidationArray
  //         );
  ...
  //         assert.include(
  //           error.message,
  //           "VesselManagerOperations__InvalidArraySize()"
  //         );
  //       }
  //     });
  ...
  //       // Carol liquidated, and her vessel is closed
  //       const txCarolClose_Asset = await borrowerOperations.closeVessel(
  //         erc20.address,
  //         {
  //           from: carol,
  //         }
  ...
  //     it("batchLiquidateVessels: when SP > 0, triggers LNDR reward event - increases the sum G", async () => {
  //       await openVessel({
  //         asset: erc20.address,
  //         ICR: toBN(dec(100, 18)),
  //         extraParams: { from: whale },
  ...
  //     });
    ...
    describe("Redemptions", async () => {
      it("getRedemptionHints(): gets the address of the first Vessel and the final ICR of the last Vessel involved in a redemption", async () => {
        ...
        await priceFeed.setPrice(erc20.address, toBN(dec(100000, 18)));
        let price = await priceFeed.getPrice(erc20.address);
        ...
        const { collateral: B_coll, netDebt: B_debt } = await openVessel({
          asset: erc20.address,
          ICR: toBN(dec(290, 16)),
          extraDebtTokenAmount: toBN(dec(100, 18)),
          extraParams: { from: bob },
        });
        const { collateral: C_coll, netDebt: C_debt } = await openVessel({
          asset: erc20.address,
          ICR: toBN(dec(250, 16)),
          extraDebtTokenAmount: toBN(dec(100, 18)),
          extraParams: { from: carol },
        ...
        console.log("price", ethers.utils.formatUnits(price.toString(), 18));

        console.log("partialRedemptionAmount", ethers.utils.formatUnits(partialRedemptionAmount.toString(), 18));

        console.log("\nA_address", alice);
        console.log("A_coll", ethers.utils.formatUnits(A_coll.toString(), 18));
        console.log("A_totalDebt", ethers.utils.formatUnits(A_totalDebt.toString(), 18));

        console.log("\nB_address", bob);
        console.log("B_coll", ethers.utils.formatUnits(B_coll.toString(), 18));
        console.log("B_Debt", ethers.utils.formatUnits(B_debt.toString(), 18));
        
        console.log("\nC_address", carol);
        console.log("C_coll", ethers.utils.formatUnits(C_coll.toString(), 18));
        console.log("C_debt", ethers.utils.formatUnits(C_debt.toString(), 18));
        ...
        console.log("redemptionAmount", ethers.utils.formatUnits(redemptionAmount.toString(), 18));
        console.log("\n");

        const { 0: firstRedemptionHint, 1: partialRedemptionHintNewICR } =
          await vesselManagerOperations.getRedemptionHints(
            erc20.address,
            partialRedemptionAmount,
            price,
            0
        ...
        console.log("partialRedemptionHintNewICR", ethers.utils.formatUnits(partialRedemptionHintNewICR.toString(), 18));
        
        const { 0: upperHint, 1: lowerHint } =
          await sortedVessels.findInsertPosition(
            erc20.address,
      ...
      //   const { 1: partialRedemptionHintNICR_Asset } =
      //     await vesselManagerOperations.getRedemptionHints(
      //       erc20.address,
      //       "210" + _18_zeros,
      //       price,
      ...
      //   await impersonateAccount(shortTimelock.address);
      //   let tx = vesselManagerOperations.setRedemptionSofteningParam(
      //     "10100",
      //     { from: shortTimelock.address }
      //   );
      //   await th.assertRevert(tx);
      //   let tx2 = vesselManagerOperations.setRedemptionSofteningParam(
      //     "9600",
      //     { from: shortTimelock.address }
      //   );
      ...
      //   await adminContract.setRedemptionBlockTimestamp(
      //     erc20.address,
      //     redemptionBlock
      //   );
    ...
    //     const dennis_DebtTokenBalance_Before = await debtToken.balanceOf(
    //       dennis
    //     );
    ...
    //     const alice_Vessel_After = await vesselManager.Vessels(
    //       alice,
    //       erc20.address
    //     );
    //     const bob_Vessel_After = await vesselManager.Vessels(
    //       bob,
    //       erc20.address
    //     );
    //     const carol_Vessel_After = await vesselManager.Vessels(
    //       carol,
    //       erc20.address
    //     );

    //     const alice_debt_After =
    //       alice_Vessel_After[th.VESSEL_DEBT_INDEX].toString();
    //     const bob_debt_After =
    //       bob_Vessel_After[th.VESSEL_DEBT_INDEX].toString();
    //     const carol_debt_After =
    //       carol_Vessel_After[th.VESSEL_DEBT_INDEX].toString();
    ...
    //     const receivedColl = dennis_CollBalance_After.sub(
    //       dennis_CollBalance_Before
    //     );

    //     const expectedTotalCollDrawn = calcSoftnedAmount(
    //       redemptionAmount,
    //       price
    //     );
    ...
    //     const dennis_DebtTokenBalance_After = (
    //       await debtToken.balanceOf(dennis)
    //     ).toString();
    //     th.assertIsApproximatelyEqual(
    ...
    //     const { 1: partialRedemptionHintNewICR } =
    //       await vesselManagerOperations.getRedemptionHints(
    //         erc20.address,
    //         redemptionAmount,
    //         price,
    ...
    //     const { debtTokenAmount: F_DebtAmount } = await openVessel({
    //       asset: erc20.address,
    //       ICR: toBN(dec(200, 18)),
    ...
    //     th.assertIsApproximatelyEqual(
    //       flynBalance,
    //       F_DebtAmount.sub(redemptionAmount)
    //     );

    //     // Check debt of Alice, Bob, Carol
    //     const alice_Debt = await vesselManager.getVesselDebt(
    //       erc20.address,
    //       alice
    //     );
    //     const bob_Debt = await vesselManager.getVesselDebt(erc20.address, bob);
    //     const carol_Debt = await vesselManager.getVesselDebt(
    //       erc20.address,
    //       carol
    //     );
    ...
    //     const alice_Status = await vesselManager.getVesselStatus(
    //       erc20.address,
    //       alice
    //     );
    //     const bob_Status = await vesselManager.getVesselStatus(
    //       erc20.address,
    //       bob
    //     );
    //     const carol_Status = await vesselManager.getVesselStatus(
    //       erc20.address,
    //       carol
    //     );
    ...
    //     const dennis_Debt = await vesselManager.getVesselDebt(
    //       erc20.address,
    //       dennis
    //     );
    //     const erin_Debt = await vesselManager.getVesselDebt(
    //       erc20.address,
    //       erin
    //     );
    ...
    //     const dennis_Coll = await vesselManager.getVesselColl(
    //       erc20.address,
    //       dennis
    //     );
    //     const erin_Coll = await vesselManager.getVesselColl(
    //       erc20.address,
    //       erin
    //     );
    ...
    //       extraParams: { from: bob },
    //     });
    //     const { netDebt: C_debt_Asset, totalDebt: C_totalDebt_Asset } =
    //       await openVessel({
    //         asset: erc20.address,
    //         ICR: toBN(dec(286, 16)),
    //         extraDebtTokenAmount: dec(20, 18),
    ...
    //     const attemptedRedemptionAmount_Asset =
    //       redemptionAmount_Asset.add(C_debt_Asset);

    //     // --- TEST ---

    //     // open vessel from redeemer.  Redeemer has highest ICR (100ETH, 100 DebtToken), 20000%
    //     const { debtTokenAmount: F_DebtTokenAmount_Asset } = await openVessel({
    //       asset: erc20.address,
    //       ICR: toBN(dec(200, 18)),
    //       extraDebtTokenAmount: redemptionAmount_Asset.mul(toBN(2)),
    //       extraParams: { from: flyn },
    //     });
    ...
    //     th.assertIsApproximatelyEqual(
    //       flynBalance,
    //       F_DebtTokenAmount_Asset.sub(redemptionAmount_Asset)
    //     );

    //     // Check debt of Alice, Bob, Carol

    //     const alice_Debt_Asset = await vesselManager.getVesselDebt(
    //       erc20.address,
    //       alice
    //     );
    //     const bob_Debt_Asset = await vesselManager.getVesselDebt(
    //       erc20.address,
    //       bob
    //     );
    //     const carol_Debt_Asset = await vesselManager.getVesselDebt(
    //       erc20.address,
    //       carol
    //     );
    ...
    //     const alice_Status_Asset = await vesselManager.getVesselStatus(
    //       erc20.address,
    //       alice
    //     );
    //     const bob_Status_Asset = await vesselManager.getVesselStatus(
    //       erc20.address,
    //       bob
    //     );
    //     const carol_Status_Asset = await vesselManager.getVesselStatus(
    //       erc20.address,
    //       carol
    //     );
    ...
    //       erc20.address,
    //       dec(1_000, "ether"),
    //       await getOpenVesselDebtTokenAmount(dec(10_000, 18), erc20.address),
    //       A,
    //       A,
    //       { from: A }
    //     );
    ...
    //       erc20.address,
    //       dec(1_000, "ether"),
    //       await getOpenVesselDebtTokenAmount(dec(20_000, 18), erc20.address),
    //       B,
    //       B,
    //       { from: B }
    //     );
    ...
    //       erc20.address,
    //       dec(1_000, "ether"),
    //       await getOpenVesselDebtTokenAmount(dec(30_000, 18), erc20.address),
    //       C,
    //       C,
    //       { from: C }
    //     );
    ...
    //     const A_debt_Asset = await vesselManager.getVesselDebt(
    //       erc20.address,
    //       A
    //     );
    ...
    //       erc20.address,
    //       dec(1000, "ether"),
    //       await getOpenVesselDebtTokenAmount(dec(6000, 18), erc20.address),
    //       A,
    //       A,
    //       { from: A }
    //     );
    ...
    //       erc20.address,
    //       dec(1000, "ether"),
    //       await getOpenVesselDebtTokenAmount(dec(20000, 18), erc20.address),
    //       B,
    //       B,
    //       { from: B }
    //     );
    ...
    //       erc20.address,
    //       dec(1000, "ether"),
    //       await getOpenVesselDebtTokenAmount(dec(30000, 18), erc20.address),
    //       C,
    //       C,
    //       { from: C }
    //     );
    ...
    //     const expectedTotalCollDrawn = calcSoftnedAmount(
    //       fullfilledRedemptionAmount.sub(frontRunRedemption),
    //       price
    //     );
    ...
    //     th.assertIsApproximatelyEqual(
    //       dennis_DebtTokenBalance_After,
    //       dennis_DebtTokenBalance_Before.sub(
    //         fullfilledRedemptionAmount.sub(frontRunRedemption)
    //       )
    //     );
    //   });
    ...
    //     const amount = await getOpenVesselDebtTokenAmount(dec(110, 18));
    ...
    //     const carol_ETHBalance_Before_Asset = toBN(
    //       await erc20.balanceOf(carol)
    //     );
    ...
    //     const ETHFee_Asset =
    //       th.getEmittedRedemptionValues(redemptionTx_Asset)[3];

    //     const carol_ETHBalance_After_Asset = toBN(await erc20.address(carol));

    //     const expectedTotalETHDrawn = toBN(amount).div(toBN(100)); // convert 100 DebtToken to ETH at ETH:USD price of 100
    //     const expectedReceivedETH_Asset =
    //       expectedTotalETHDrawn.sub(ETHFee_Asset);

    //     const receivedETH_Asset = carol_ETHBalance_After_Asset.sub(
    //       carol_ETHBalance_Before_Asset
    //     );
    //     assert.isTrue(expectedReceivedETH_Asset.eq(receivedETH_Asset));

    //     const carol_DebtTokenBalance_After = (
    //       await debtToken.balanceOf(carol)
    //     ).toString();
    ...
    //     });
    //     const {
    //       debtTokenAmount: B_DebtTokenAmount_Asset,
    //       totalDebt: B_totalDebt_Asset,
    //     } = await openVessel({
    //       asset: erc20.address,
    //       ICR: toBN(dec(133, 16)),
    //       extraDebtTokenAmount: A_debt_Asset,
    //       extraParams: { from: bob },
    //     });

    //     await debtToken.transfer(carol, B_DebtTokenAmount_Asset, { from: bob });
    ...
    //     const { debt: alice_Debt_After_Asset } = await vesselManager.Vessels(
    //       alice,
    //       erc20.address
    //     );
    ...
    //     const { debt: bob_Debt_After_Asset } = await vesselManager.Vessels(
    //       bob,
    //       erc20.address
    //     );
    ...
    //     const redemptionAmount_Asset =
    //       C_totalDebt_Asset.add(B_totalDebt_Asset).add(A_totalDebt_Asset);
    //     const { totalDebt: D_totalDebt_Asset } = await openVessel({
    //       asset: erc20.address,
    //       ICR: toBN(dec(195, 16)),
    //       extraDebtTokenAmount: redemptionAmount_Asset,
    //       extraParams: { from: dennis },
    //     });
    ...
    //       current_Asset = await sortedVessels.getNext(
    //         erc20.address,
    //         current_Asset
    //       );
    //     }
    ...
    //     const { debt: carol_Debt_After_Asset } = await vesselManager.Vessels(
    //       carol,
    //       erc20.address
    //     );
    //     assert.equal(carol_Debt_After_Asset, "0");

    //     const { debt: dennis_Debt_After_Asset } = await vesselManager.Vessels(
    //       dennis,
    //       erc20.address
    //     );
    //     th.assertIsApproximatelyEqual(
    //       dennis_Debt_After_Asset,
    //       D_totalDebt_Asset
    //     );
    //   });
    ...
    //     // Alice opens vessel and transfers 500DebtToken to Erin, the would-be redeemer
    //     await openVessel({
    //       asset: erc20.address,
    //       ICR: toBN(dec(200, 16)),
    ...
    //     const redemptionTxPromise_Asset =
    ...
    //       );
    //     await assertRevert(
    //       redemptionTxPromise_Asset,
    //       "VesselManager: Amount must be greater than zero"
    //     );
    //   });
    ...
    //     const expectedTotalSupply_Asset =
    ...
    //     const attemptedDebtTokenRedemption_Asset =
    //       expectedTotalSupply_Asset.div(toBN(10));
    ...
    //         A,
    //         contracts.core,
    //         attemptedDebtTokenRedemption_Asset,
    //         erc20.address,
    //         lessThan5pct
    //       ),
    ...
    //     const price = await priceFeed.getPrice(erc20.address);
    //     const ETHDrawn_Asset = attemptedDebtTokenRedemption_Asset
    //       .mul(mv._1e18BN)
    //       .div(price);

    //     const slightlyMoreThanFee_Asset =
    //       await vesselManager.getRedemptionFeeWithDecay(
    //         erc20.address,
    //         ETHDrawn_Asset
    //       );
    ...
    //     // Alice opens vessel and transfers DebtToken to Erin, the would-be redeemer
    //     await openVessel({
    //       asset: erc20.address,
    //       ICR: toBN(dec(300, 16)),
    //       extraDebtTokenAmount: redemptionAmount_Asset,
    ...
    //       from: bob,
    //     });
    //     await stakedDebtToken.deposit(dec(50, 18), bob, {
    //       from: bob,
    //     });

    //     await debtToken.approve(stakedDebtToken.address, dec(150, 18), {
    //       from: carol,
    //     });
    //     await stakedDebtToken.deposit(dec(150, 18), carol, {
    //       from: carol,
    //     });

    //     await debtToken.approve(stakedDebtToken.address, dec(200, 18), {
    //       from: dennis,
    //     });
    //     await stakedDebtToken.deposit(dec(200, 18), dennis, {
    //       from: dennis,
    //     });
    ...
    //       price
    //     );
    //     const dennis_ICR_before_Asset = await vesselManager.getCurrentICR(
    //       erc20.address,
    //       dennis,
    //       price
    //     );
    ...
    //     // const bob_SPDeposit_before_Asset = (
    //     //   await stakedDebtToken.getCompoundedDebtTokenDeposits(bob)
    //     // ).toString();
    //     // const carol_SPDeposit_before_Asset = (
    //     //   await stakedDebtToken.getCompoundedDebtTokenDeposits(carol)
    //     // ).toString();
    ...
    //     //   await stakedDebtToken.getDepositorGains(carol, validCollateral)
    //     // )[1][idx].toString();
    //     // const dennis_ETHGain_before_Asset = (
    //     //   await stakedDebtToken.getDepositorGains(dennis, validCollateral)
    //     // )[1][idx].toString();

    //     // // Check the remaining DebtToken and ETH in Stability Pool after liquidation is non-zero

    //     // const DebtTokeninSP_Asset = await stakedDebtToken.totalAssets();
    //     // const ETHinSP_Asset = await stakedDebtToken.getCollateral(erc20.address);

    //     // assert.isTrue(DebtTokeninSP_Asset.gte(mv._zeroBN));
    ...
    //       price
    //     );
    //     const dennis_ICR_after_Asset = await vesselManager.getCurrentICR(
    //       erc20.address,
    //       dennis,
    //       price
    //     );
    ...
    //     // const bob_SPDeposit_after_Asset = (
    //     //   await stakedDebtToken.getCompoundedDebtTokenDeposits(bob)
    //     // ).toString();
    //     // const carol_SPDeposit_after_Asset = (
    //     //   await stakedDebtToken.getCompoundedDebtTokenDeposits(carol)
    //     // ).toString();
    ...
    //     // assert.equal(
    //     //   dennis_SPDeposit_before_Asset,
    //     //   dennis_SPDeposit_after_Asset
    //     // );
    ...
    //       .add(C_totalDebt)
    //       .add(D_totalDebt);
    //     const totalColl = W_coll.add(A_coll)
    //       .add(B_coll)
    //       .add(C_coll)
    //       .add(D_coll);

    //     // Get active debt and coll before redemption
    //     const activePool_debt_before = await activePool.getDebtTokenBalance(
    //       erc20.address
    //     );
    //     const activePool_coll_before = await activePool.getAssetBalance(
    //       erc20.address
    //     );

    //     th.assertIsApproximatelyEqual(
    //       activePool_debt_before.toString(),
    //       totalDebt
    //     );
    ...
    //     const activePool_debt_after = await activePool.getDebtTokenBalance(
    //       erc20.address
    //     );
    //     assert.equal(
    ...
    //     const activePool_coll_after = await activePool.getAssetBalance(
    //       erc20.address
    //     );
    //     const expectedCollWithdrawn = calcSoftnedAmount(
    //       toBN(dec(400, 18)),
    //       price
    //     );
    //     assert.equal(
    //       activePool_coll_after.toString(),
    //       activePool_coll_before.sub(expectedCollWithdrawn)
    //     );
    ...
    //     const { collateral: W_coll_Asset, totalDebt: W_totalDebt_Asset } =
    //       await openVessel({
    //         asset: erc20.address,
    //         ICR: toBN(dec(20, 18)),
    //         extraParams: { from: whale },
    ...
    //     // Alice opens vessel and transfers 400 DebtToken to Erin, the would-be redeemer
    ...
    //         extraParams: { from: alice },
    //       });
    //     await debtToken.transfer(erin, toBN(dec(400, 18)).mul(toBN(2)), {
    //       from: alice,
    //     });
    ...
    //     const { collateral: B_coll_Asset, totalDebt: B_totalDebt_Asset } =
    //       await openVessel({
    //         asset: erc20.address,
    //         ICR: toBN(dec(300, 16)),
    //         extraDebtTokenAmount: dec(590, 18),
    //         extraParams: { from: bob },
    //       });
    //     const { collateral: C_coll_Asset, totalDebt: C_totalDebt_Asset } =
    //       await openVessel({
    //         asset: erc20.address,
    //         ICR: toBN(dec(300, 16)),
    //         extraDebtTokenAmount: dec(1990, 18),
    //         extraParams: { from: carol },
    //       });
    //     const { collateral: D_coll_Asset, totalDebt: D_totalDebt_Asset } =
    //       await openVessel({
    //         asset: erc20.address,
    //         ICR: toBN(dec(500, 16)),
    //         extraDebtTokenAmount: dec(1990, 18),
    ...
    //     const activePool_debt_before_Asset =
    //       await activePool.getDebtTokenBalance(erc20.address);
    //     const activePool_coll_before_Asset = (
    //       await activePool.getAssetBalance(erc20.address)
    //     ).toString();

    //     th.assertIsApproximatelyEqual(
    //       activePool_debt_before_Asset,
    //       totalDebt_Asset
    //     );
    ...
    //       assert.include(
    //         error.message,
    //         "VesselManagerOperations__InsufficientDebtTokenBalance"
    //       );
    //     }

    //     // Erin tries to redeem 801 DebtToken
    //     try {
    //       ({
    //         0: firstRedemptionHint_Asset,
    //         1: partialRedemptionHintNICR_Asset,
    ...
    //       const {
    //         0: upperPartialRedemptionHint_2_Asset,
    //         1: lowerPartialRedemptionHint_2_Asset,
    //       } = await sortedVessels.findInsertPosition(
    //         erc20.address,
    //         partialRedemptionHintNICR_Asset,
    //         erin,
    ...
    //       const {
    //         0: upperPartialRedemptionHint_3_Asset,
    //         1: lowerPartialRedemptionHint_3_Asset,
    //       } = await sortedVessels.findInsertPosition(
    //         erc20.address,
    //         partialRedemptionHintNICR_Asset,
    //         erin,
    ...
    //       const {
    //         0: upperPartialRedemptionHint_4_Asset,
    //         1: lowerPartialRedemptionHint_4_Asset,
    //       } = await sortedVessels.findInsertPosition(
    //         erc20.address,
    //         partialRedemptionHintNICR_Asset,
    //         erin,
    ...
    //       extraParams: { from: alice },
    //     });
    //     await debtToken.transfer(erin, toBN(dec(1_000, 18)).mul(toBN(2)), {
    //       from: alice,
    //     });
    //     await debtToken.transfer(flyn, toBN(dec(1_000, 18)).mul(toBN(2)), {
    //       from: alice,
    //     });
    //     await debtToken.transfer(graham, toBN(dec(1_000, 18)).mul(toBN(2)), {
    //       from: alice,
    //     });
    ...
    //       asset: erc20.address,
    //       ICR: toBN(dec(300, 16)),
    //       extraDebtTokenAmount: dec(1_590, 18),
    //       extraParams: { from: bob },
    //     });
    //     const { collateral: C_coll } = await openVessel({
    //       asset: erc20.address,
    //       ICR: toBN(dec(600, 16)),
    //       extraDebtTokenAmount: dec(1_090, 18),
    //       extraParams: { from: carol },
    //     });
    ...
    //     const activePoolBalance0 = await activePool.getAssetBalance(
    //       erc20.address
    //     );
    ...
    //     const activePoolBalance1 = await activePool.getAssetBalance(
    //       erc20.address
    //     );
    //     const expectedActivePoolBalance1 = activePoolBalance0.sub(
    //       calcSoftnedAmount(toBN(_120_), price)
    //     );
    //     assert.equal(
    //       activePoolBalance1.toString(),
    //       expectedActivePoolBalance1.toString()
    //     );

    //     // Flyn redeems 373 debt tokens
    //     ({ 0: firstRedemptionHint, 1: partialRedemptionHintNewICR } =
    //       await vesselManagerOperations.getRedemptionHints(
    //         erc20.address,
    //         _373_,
    //         price,
    ...
    //     const activePoolBalance2 = await activePool.getAssetBalance(
    //       erc20.address
    //     );
    //     const expectedActivePoolBalance2 = activePoolBalance1.sub(
    //       calcSoftnedAmount(toBN(_373_), price)
    //     );
    //     assert.equal(
    //       activePoolBalance2.toString(),
    //       expectedActivePoolBalance2.toString()
    //     );
    ...
    //     const activePoolBalance3 = (
    //       await activePool.getAssetBalance(erc20.address)
    //     ).toString();
    //     const expectedActivePoolBalance3 = activePoolBalance2.sub(
    //       calcSoftnedAmount(toBN(_950_), price)
    //     );
    //     assert.equal(
    //       activePoolBalance3.toString(),
    //       expectedActivePoolBalance3.toString()
    //     );
    //   });
    ...
    //     const {
    //       0: upperPartialRedemptionHint_Asset,
    //       1: lowerPartialRedemptionHint_Asset,
    //     } = await sortedVessels.findInsertPosition(
    //       erc20.address,
    //       partialRedemptionHintNICR_Asset,
    //       bob,
    ...
    //     th.assertIsApproximatelyEqual(
    //       (await activePool.getDebtTokenBalance(erc20.address)).toString(),
    //       totalDebt_Asset
    //     );
    ...
    //     assert.equal(
    //       await debtToken.balanceOf(A),
    //       A_balanceBefore.sub(toBN(dec(10, 18))).toString()
    //     );

    //     // Check baseRate is now non-zero
    //     assert.isTrue(
    //       (await vesselManager.baseRate(erc20.address)).gt(toBN("0"))
    //     );
    //   });
    ...
    //     // time fast-forwards 1 year, and multisig stakes 1 LNDR
    //     await th.fastForwardTime(
    //       timeValues.SECONDS_IN_ONE_YEAR,
    //       web3.currentProvider
    //     );
    //     await lndrToken.approve(lndrStaking.address, dec(1, 18), {
    //       from: multisig,
    //     });
    //     await lndrStaking.stake(dec(1, 18), { from: multisig });
    ...
    //       erc20.address
    //     );
    //     const timeStamp_A_Asset = await th.getTimestampFromTx(
    //       redemptionTx_A_Asset,
    //       web3
    //     );
    ...
    //       erc20.address
    //     );
    //     const timeStamp_B_Asset = await th.getTimestampFromTx(
    //       redemptionTx_B_Asset,
    //       web3
    //     );

    //     // Check B's balance has decreased by 10 DebtToken
    //     assert.equal(
    //       await debtToken.balanceOf(B),
    //       B_balanceBefore.sub(toBN(dec(10, 18))).toString()
    //     );

    //     // Check negligible time difference (< 1 minute) between txs
    //     assert.isTrue(
    //       Number(timeStamp_B_Asset) - Number(timeStamp_A_Asset) < 60
    //     );
    ...
    //     // Check A's balance has decreased by 10 DebtToken
    //     assert.equal(
    //       A_balanceBefore.sub(await debtToken.balanceOf(A)).toString(),
    //       toBN(dec(10, 18)).toString()
    //     );
    ...
    //     const lastFeeOpTime_1_Asset = await vesselManager.lastFeeOperationTime(
    //       erc20.address
    //     );
    ...
    //     const lastFeeOpTime_2_Asset = await vesselManager.lastFeeOperationTime(
    //       erc20.address
    //     );
    ...
    //     const lastFeeOpTime_3_Asset = await vesselManager.lastFeeOperationTime(
    //       erc20.address
    //     );
    ...
    //   it("redeemCollateral(): a redemption made at zero base rate send a non-zero ETHFee to LNDR staking contract", async () => {
    //     // time fast-forwards 1 year, and multisig stakes 1 LNDR
    //     await th.fastForwardTime(
    //       timeValues.SECONDS_IN_ONE_YEAR,
    //       web3.currentProvider
    ...
    //     // Check LNDR Staking contract balance after is non-zero

    //     const treasuryBalanceAfter = await erc20.balanceOf(treasury);
    //     assert.isTrue(treasuryBalanceAfter.gt(treasuryBalanceBefore));
    //   });

    //   it("redeemCollateral(): a redemption made at zero base increases the ETH-fees-per-LNDR-staked in LNDR Staking contract", async () => {
    //     // time fast-forwards 1 year, and multisig stakes 1 LNDR
    //     await th.fastForwardTime(
    //       timeValues.SECONDS_IN_ONE_YEAR,
    //       web3.currentProvider
    ...
    //     // Check LNDR Staking ETH-fees-per-LNDR-staked before is zero
    //     assert.equal(await lndrStaking.F_ASSETS(erc20.address), "0");
    ...
    //     // Check LNDR Staking ETH-fees-per-LNDR-staked after is non-zero
    //     assert.isTrue((await lndrStaking.F_ASSETS(erc20.address)).gt("0"));
    //   });

    //   it("redeemCollateral(): a redemption made at a non-zero base rate send a non-zero ETHFee to LNDR staking contract", async () => {
    //     // time fast-forwards 1 year, and multisig stakes 1 LNDR
    //     await th.fastForwardTime(
    //       timeValues.SECONDS_IN_ONE_YEAR,
    //       web3.currentProvider
    ...
    //     // check LNDR Staking balance has increased
    //     assert.isTrue(treasuryBalanceAfter.gt(treasuryBalanceBefore));
    //   });

    //   it("redeemCollateral(): a redemption made at a non-zero base rate increases ETH-per-LNDR-staked in the staking contract", async () => {
    //     // time fast-forwards 1 year, and multisig stakes 1 LNDR
    //     await th.fastForwardTime(
    //       timeValues.SECONDS_IN_ONE_YEAR,
    //       web3.currentProvider
    ...
    //     // Check total debtToken supply
    //     const debtTokensOnActivePool = await activePool.getDebtTokenBalance(
    //       erc20.address
    //     );
    //     const debtTokensOnDefaultPool = await defaultPool.getDebtTokenBalance(
    //       erc20.address
    //     );

    //     const totalDebtTokenSupply = debtTokensOnActivePool.add(
    //       debtTokensOnDefaultPool
    //     );
    ...
    //       extraParams: { from: whale },
    //     });
    //     const { netDebt: A_netDebt_Asset, collateral: A_coll_Asset } =
    //       await openVessel({
    //         asset: erc20.address,
    //         ICR: toBN(dec(200, 16)),
    //         extraDebtTokenAmount: dec(100, 18),
    //         extraParams: { from: A },
    //       });
    //     const { netDebt: B_netDebt_Asset, collateral: B_coll_Asset } =
    //       await openVessel({
    //         asset: erc20.address,
    //         ICR: toBN(dec(190, 16)),
    //         extraDebtTokenAmount: dec(100, 18),
    //         extraParams: { from: B },
    //       });
    //     const { netDebt: C_netDebt_Asset, collateral: C_coll_Asset } =
    //       await openVessel({
    //         asset: erc20.address,
    //         ICR: toBN(dec(180, 16)),
    //         extraDebtTokenAmount: dec(100, 18),
    ...
    //     const A_collBefore_Asset = await vesselManager.getVesselColl(
    //       erc20.address,
    //       A
    //     );
    //     const B_collBefore_Asset = await vesselManager.getVesselColl(
    //       erc20.address,
    //       B
    //     );
    //     const C_collBefore_Asset = await vesselManager.getVesselColl(
    //       erc20.address,
    //       C
    //     );
    //     const D_collBefore_Asset = await vesselManager.getVesselColl(
    //       erc20.address,
    //       D
    //     );
    ...
    //     const A_collAfter_Asset = await vesselManager.getVesselColl(
    //       erc20.address,
    //       A
    //     );
    //     const B_collAfter_Asset = await vesselManager.getVesselColl(
    //       erc20.address,
    //       B
    //     );
    //     const C_collAfter_Asset = await vesselManager.getVesselColl(
    //       erc20.address,
    //       C
    //     );
    ...
    //     const D_collAfter_Asset = await vesselManager.getVesselColl(
    //       erc20.address,
    //       D
    //     );
    ...
    //       extraParams: { from: C },
    //     });
    //     const { totalDebt: D_totalDebt_Asset, collateral: D_coll_Asset } =
    //       await openVessel({
    //         asset: erc20.address,
    //         ICR: toBN(dec(280, 16)),
    //         extraDebtTokenAmount: dec(100, 18),
    ...
    //       A_balanceExpected_Asset
    //     );
    //     th.assertIsApproximatelyEqual(
    //       B_balanceAfter_Asset,
    //       B_balanceExpected_Asset
    //     );
    //     th.assertIsApproximatelyEqual(
    //       C_balanceAfter_Asset,
    //       C_balanceExpected_Asset
    //     );
    //   });
    ...
    //     const A_surplus_Asset = A_collBefore_Asset.sub(
    //       calcSoftnedAmount(A_netDebt_Asset, price)
    //     );
    //     const B_surplus_Asset = B_collBefore_Asset.sub(
    //       calcSoftnedAmount(B_netDebt_Asset, price)
    //     );
    //     const C_surplus_Asset = C_collBefore_Asset.sub(
    //       calcSoftnedAmount(C_netDebt_Asset, price)
    //     );
    ...
    //     th.assertIsApproximatelyEqual(
    //       A_balanceAfter_Asset,
    //       A_balanceBefore_Asset.add(A_surplus_Asset)
    //     );
    //     th.assertIsApproximatelyEqual(
    //       B_balanceAfter_Asset,
    //       B_balanceBefore_Asset.add(B_surplus_Asset)
    //     );
    //     th.assertIsApproximatelyEqual(
    //       C_balanceAfter_Asset,
    //       C_balanceBefore_Asset.add(C_surplus_Asset)
    //     );
    //   });
    ...
    //     const { debtTokenAmount: DebtTokenAmount_Asset } = await openVessel({
    //       asset: erc20.address,
    //       ICR: toBN(dec(200, 16)),
    //       extraDebtTokenAmount: dec(1, 24),
    //       extraParams: { from: alice },
    ...
    //         erc20.address,
    //         DebtTokenAmount_Asset.mul(mv._1e18BN).div(price),
    //         0,
    //         DebtTokenAmount_Asset,
    //         true,
    //         alice,
    ...
    //   it("getPendingDebtTokenReward(): returns 0 if there is no pending DebtTokenDebt reward", async () => {
    //     // Make some vessels
    ...
    //       asset: erc20.address,
    //       ICR: toBN(dec(20, 18)),
    //       extraDebtTokenAmount: totalDebt_Asset,
    //       extraParams: { from: whale },
    //     });

    //     await debtToken.approve(stakedDebtToken.address, totalDebt_Asset, {
    //       from: whale,
    //     });
    //     await stakedDebtToken.deposit(totalDebt_Asset, whale, {
    //       from: whale,
    //     });
    ...
    //     const current_L_DebtTokenDebt_Asset = await vesselManager.L_Debts(
    //       erc20.address
    //     );
    //     assert.equal(current_L_DebtTokenDebt_Asset, 0);

    //     const carolSnapshot_L_DebtTokenDebt_Asset = (
    //       await vesselManager.rewardSnapshots(carol, erc20.address)
    //     )[1];
    //     assert.equal(carolSnapshot_L_DebtTokenDebt_Asset, 0);

    //     const carol_PendingDebtTokenDebtReward_Asset =
    //       await vesselManager.getPendingDebtTokenReward(erc20.address, carol);
    //     assert.equal(carol_PendingDebtTokenDebtReward_Asset, 0);
    //   });
    ...
    //     const carolSnapshot_L_ETH_Asset = (
    //       await vesselManager.rewardSnapshots(carol, erc20.address)
    //     )[0];
    ...
    //     const ICR = (
    //       await vesselManager.computeICR(coll, debt, price)
    //     ).toString();
    //     assert.equal(ICR, 0);
    //   });

    //   it("computeICR(): returns 2^256-1 for ETH:USD = 100, coll = 1 ETH, debt = 100 DebtToken", async () => {
    //     const price = dec(100, 18);
    //     const coll = dec(1, "ether");
    //     const debt = dec(100, 18);
    ...
    //   it("computeICR(): returns correct ICR for ETH:USD = 100, coll = 200 ETH, debt = 30 DebtToken", async () => {
    //     const price = dec(100, 18);
    ...
    //   it("computeICR(): returns correct ICR for ETH:USD = 250, coll = 1350 ETH, debt = 127 DebtToken", async () => {
    ...
    //     assert.isAtMost(
    //       th.getDifference(ICR, "2657480314960630000000"),
    //       1000000
    //     );
    //   });

    //   it("computeICR(): returns correct ICR for ETH:USD = 100, coll = 1 ETH, debt = 54321 DebtToken", async () => {
    //     const price = dec(100, 18);
    //     const coll = dec(1, "ether");
    ...
    //     const A_Stake_Asset = await vesselManager.getVesselStake(
    //       erc20.address,
    //       A
    //     );
    //     const B_Stake_Asset = await vesselManager.getVesselStake(
    //       erc20.address,
    //       B
    //     );
    ...
    //     assert.equal(
    //       await vesselManager.getVesselColl(erc20.address, A),
    //       A_coll_Asset.toString()
    //     );
    //     assert.equal(
    //       await vesselManager.getVesselColl(erc20.address, B),
    //       B_coll_Asset.toString()
    //     );
    //   });
    ...
    //       A
    //     );
    //     const B_Debt_Asset = await vesselManager.getVesselDebt(
    //       erc20.address,
    //       B
    //     );
    ...
    //       asset: erc20.address,
    //       ICR: toBN(dec(150, 16)),
    //       extraDebtTokenAmount: B_totalDebt_Asset,
    //       extraParams: { from: A },
    //     });
    ...
    //     const A_Status_Asset = await vesselManager.getVesselStatus(
    //       erc20.address,
    //       A
    //     );
    //     const B_Status_Asset = await vesselManager.getVesselStatus(
    //       erc20.address,
    //       B
    //     );
    //     const C_Status_Asset = await vesselManager.getVesselStatus(
    //       erc20.address,
    //       C
    //     );
    ...
    //     assert.isFalse(
    //       await vesselManager.hasPendingRewards(erc20.address, alice)
    //     );
    //   });
    // });
  });
