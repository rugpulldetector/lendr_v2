...
const deploy = async (treasury, mintingAccounts) => {
	...
	inflationFeed = contracts.core.inflationFeedTester
...
contract("BorrowerOperations", async accounts => {
	...
	describe("BorrowerOperations Mechanisms", async () => {
		before(async () => {
			...
			await feeCollector.setRouteToLNDRStaking(true) // sends fees to LNDRStaking instead of treasury
			...
			await lndrToken.unprotectedMint(multisig, dec(5, 24))

			for (const acc of accounts.slice(0, 20)) {
				await lndrToken.approve(lndrStaking.address, await web3.eth.getBalance(acc), { from: acc })
	...
	// 	// --- withdrawDebtTokens() ---
		...
		it("withdrawDebtTokens(): borrowing at non-zero base rate sends fee to LNDR staking contract", async () => {
			// time fast-forwards 1 year, and multisig stakes 1 LNDR
			await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
			await lndrToken.approve(lndrStaking.address, dec(1, 18), { from: multisig })
			await lndrStaking.stake(dec(1, 18), { from: multisig })

			// Check LNDR GRAI balance before == 0
			const LNDRStaking_DebtTokenBalance_Before = await debtToken.balanceOf(lndrStaking.address)
			assert.equal(LNDRStaking_DebtTokenBalance_Before, "0")
			...
			let feeCollectorFromContract = await borrowerOperations.feeCollector()
			
			// Check LNDR GRAI balance after has increased
			assert.equal(feeCollectorFromContract.toString().toLowerCase(), feeCollector.address.toString().toLowerCase())

			let stakingFromContract = await feeCollector.lndrStaking()
			assert.equal(stakingFromContract.toString().toLowerCase(), lndrStaking.address.toString().toLowerCase())

			const LNDRStaking_DebtTokenBalance_After = await debtToken.balanceOf(lndrStaking.address)
		
			assert.isTrue(LNDRStaking_DebtTokenBalance_After.gt(LNDRStaking_DebtTokenBalance_Before))
		})
		...
		it("withdrawDebtTokens(): borrowing at non-zero base rate increases the LNDR staking contract GRAI fees-per-unit-staked", async () => {
			// time fast-forwards 1 year, and multisig stakes 1 LNDR
			await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
			await lndrToken.approve(lndrStaking.address, dec(1, 18), { from: multisig })
			await lndrStaking.stake(dec(1, 18), { from: multisig })

			// Check LNDR contract GRAI fees-per-unit-staked is zero
			const F_DEBT_TOKEN_Before = await lndrStaking.F_DEBT_TOKENS()
			assert.equal(F_DEBT_TOKEN_Before, "0")
			...
			// Check LNDR contract GRAI fees-per-unit-staked has increased
			const F_DEBT_TOKEN_After = await lndrStaking.F_DEBT_TOKENS()
			assert.isTrue(F_DEBT_TOKEN_After.gt(F_DEBT_TOKEN_Before))
		})
		...
		it("withdrawDebtTokens(): borrowing at non-zero base rate sends requested amount to the user", async () => {
			...
			// Check LNDR Staking contract balance before == 0
			const LNDRStaking_DebtTokenBalance_Before = await debtToken.balanceOf(lndrStaking.address)
			assert.equal(LNDRStaking_DebtTokenBalance_Before, "0")
			...
			let D_DebtTokenBalanceBefore = await debtToken.balanceOf(D)

			// D withdraws GRAI
			const D_DebtTokenRequest = toBN(dec(37, 18))
			await borrowerOperations.withdrawDebtTokens(erc20.address, D_DebtTokenRequest, D, D, {
				from: D,
			})

			// Check LNDR staking GRAI balance has increased
			let LNDRStaking_DebtTokenBalance_After = await debtToken.balanceOf(lndrStaking.address)
			assert.isTrue(LNDRStaking_DebtTokenBalance_After.gt(LNDRStaking_DebtTokenBalance_Before))

			// Check D's GRAI balance now equals their initial balance plus request GRAI
			let D_DebtTokenBalanceAfter = await debtToken.balanceOf(D)
			assert.isTrue(D_DebtTokenBalanceAfter.eq(D_DebtTokenBalanceBefore.add(D_DebtTokenRequest)))

			//Asset:
			D_DebtTokenBalanceBefore = await debtToken.balanceOf(D)
			await borrowerOperations.withdrawDebtTokens(erc20.address, D_DebtTokenRequest, D, D, {
				from: D,
			})

			LNDRStaking_DebtTokenBalance_After = await debtToken.balanceOf(lndrStaking.address)
			assert.isTrue(LNDRStaking_DebtTokenBalance_After.gt(LNDRStaking_DebtTokenBalance_Before))

			D_DebtTokenBalanceAfter = await debtToken.balanceOf(D)
			assert.isTrue(D_DebtTokenBalanceAfter.eq(D_DebtTokenBalanceBefore.add(D_DebtTokenRequest)))
		})
		...
		it("withdrawDebtTokens(): borrowing at zero base rate changes GRAI fees-per-unit-staked", async () => {
			...
			// A artificially receives LNDR, then stakes it
			await lndrToken.unprotectedMint(A, dec(100, 18))
			await lndrStaking.stake(dec(100, 18), { from: A })
			...
			// Check LNDR GRAI balance after > 0
			const F_DEBT_TOKEN_After = await lndrStaking.F_DEBT_TOKENS()
			assert.isTrue(F_DEBT_TOKEN_After.gt("0"))
		})
		...
		it("adjustVessel(): borrowing at non-zero base rate sends GRAI fee to LNDR staking contract", async () => {
			// time fast-forwards 1 year, and multisig stakes 1 LNDR
			await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
			await lndrToken.approve(lndrStaking.address, dec(1, 18), { from: multisig })
			await lndrStaking.stake(dec(1, 18), { from: multisig })
		...
		it("adjustVessel(): borrowing at non-zero base rate increases the LNDR staking contract GRAI fees-per-unit-staked", async () => {
			// time fast-forwards 1 year, and multisig stakes 1 LNDR
			await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
			await lndrToken.approve(lndrStaking.address, dec(1, 18), { from: multisig })
			await lndrStaking.stake(dec(1, 18), { from: multisig })
			...
			// D adjusts vessel
			const F_DEBT_TOKEN_BeforeAdjust = await lndrStaking.F_DEBT_TOKENS()

			await borrowerOperations.adjustVessel(erc20.address, 0, 0, dec(37, 18), true, D, D, {
				from: D,
			})

			const F_DEBT_TOKEN_After_Asset = await lndrStaking.F_DEBT_TOKENS()
			assert.isTrue(F_DEBT_TOKEN_After_Asset.gt(F_DEBT_TOKEN_BeforeAdjust))
		})
		...
		it("adjustVessel(): borrowing at non-zero base rate sends requested amount to the user", async () => {
			...
			const D_DebtTokenBalanceBefore = await debtToken.balanceOf(D)
			...
			// Check LNDR staking GRAI balance has increased
			const LNDRStaking_DebtTokenBalance_After_Asset = await debtToken.balanceOf(lndrStaking.address)
			assert.isTrue(LNDRStaking_DebtTokenBalance_After_Asset.gt(LNDRStaking_DebtTokenBalance_After))

			// Check D's GRAI balance has increased by their requested GRAI
			const D_DebtTokenBalanceAfter_Asset = await debtToken.balanceOf(D)
			assert.isTrue(D_DebtTokenBalanceAfter_Asset.eq(D_DebtTokenBalanceAfter.add(GRAIRequest_D)))
		})

		// NOTE: Logic changed
		it("adjustVessel(): borrowing at zero borrowing fee doesn't GRAI balance of LNDR staking contract", async () => {
			await openVessel({
				asset: erc20.address,
				ICR: toBN(dec(10, 18)),
				extraParams: { from: whale },
			...
			const LNDRStaking_DebtTokenBalance_Before = await debtToken.balanceOf(lndrStaking.address)
			assert.isTrue(LNDRStaking_DebtTokenBalance_Before.gt(toBN("0")))
			...
			const LNDRStaking_DebtTokenBalance_After_Asset = await debtToken.balanceOf(lndrStaking.address)
			assert.isTrue(LNDRStaking_DebtTokenBalance_After_Asset.eq(LNDRStaking_DebtTokenBalance_After))
		})
		// Changed logic
		it("adjustVessel(): borrowing at zero borrowing fee doesn't change LNDR staking contract GRAI fees-per-unit-staked", async () => {
			await openVessel({
				asset: erc20.address,
				assetSent: toBN(dec(100, "ether")),
			...
			// Check staking GRAI balance before == 0
			const F_DEBT_TOKEN_Before = await lndrStaking.F_DEBT_TOKENS()
			assert.isTrue(F_DEBT_TOKEN_Before.eq(toBN("0")))
			...
			const F_DEBT_TOKEN_After_Asset = await lndrStaking.F_DEBT_TOKENS()
			assert.isTrue(F_DEBT_TOKEN_After_Asset.eq(F_DEBT_TOKEN_After))
		})
		...
		it("adjustVessel(): debt increase in Recovery Mode charges no fee", async () => {
			...
			// B stakes LNDR
			await lndrToken.unprotectedMint(bob, dec(100, 18))
			await lndrStaking.stake(dec(100, 18), { from: bob })

			const LNDRStakingGRAIBalanceBefore = await debtToken.balanceOf(lndrStaking.address)
			assert.isTrue(LNDRStakingGRAIBalanceBefore.gt(toBN("0")))
			...
			const LNDRStakingGRAIBalanceAfter = await debtToken.balanceOf(lndrStaking.address)
			assert.equal(LNDRStakingGRAIBalanceAfter.toString(), LNDRStakingGRAIBalanceBefore.toString())
		})
		...
		it("closeVessel(): reduces ActivePool debt by correct amount", async () => {
			await openVessel({
				asset: erc20.address,
				extraDebtTokenAmount: toBN(dec(10_000, 18)),
				ICR: toBN(dec(2, 18)),
				extraParams: { from: dennis },
			})
			await openVessel({
		...
		it("openVessel(): borrowing at non-zero base rate sends GRAI fee to LNDR staking contract", async () => {
			// time fast-forwards 1 year, and multisig stakes 1 LNDR
			await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
			await lndrToken.approve(lndrStaking.address, dec(1, 18), { from: multisig })
			await lndrStaking.stake(dec(1, 18), { from: multisig })
		...
		if (!withProxy) {
			...
			it("openVessel(): borrowing at non-zero base records the (drawn debt + fee  + liq. reserve) on the Vessel struct", async () => {
				...
				th.assertIsApproximatelyEqual(
					newDebt_Asset,
					D_DebtTokenRequest.add(emittedFee_Asset).add(GRAI_GAS_COMPENSATION_ERC20),
					100000
				)
			})
		}

		it("openVessel(): borrowing at non-zero base rate increases the LNDR staking contract GRAI fees-per-unit-staked", async () => {
			// time fast-forwards 1 year, and multisig stakes 1 LNDR
			await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
			await lndrToken.approve(lndrStaking.address, dec(1, 18), { from: multisig })
			await lndrStaking.stake(dec(1, 18), { from: multisig })
		...
		// Logic changed
		it("openVessel(): borrowing at zero fee doesn't change the LNDR staking contract GRAI fees-per-unit-staked", async () => {
			await openVessel({
				asset: erc20.address,
				extraDebtTokenAmount: toBN(dec(5000, 18)),
				ICR: toBN(dec(2, 18)),
			...
			// Check GRAI reward per LNDR staked == 0
			const F_DEBT_TOKEN_Before = await lndrStaking.F_DEBT_TOKENS()
			assert.equal(F_DEBT_TOKEN_Before, "0")
			...
			// Check GRAI reward per LNDR staked > 0
			const F_DEBT_TOKEN_After = await lndrStaking.F_DEBT_TOKENS()
			assert.isTrue(F_DEBT_TOKEN_After.eq(toBN("0")))
		})
       ...
       and L_GRAI should equal 18 GRAI per-ether-staked. */
		...
		it("openVessel(): increases the Vessel's GRAI debt by the correct amount", async () => {
			...
			await borrowerOperations.openVessel(
				erc20.address,
				dec(100, "ether"),
				await getOpenVesselDebtTokenAmount(dec(10000, 18), erc20.address),
				alice,
				alice,
				{ from: alice }
			)
		...
		describe("getNewTCRFromVesselChange() returns the correct TCR", async () => {
			...
			it("collChange = 0, debtChange = 0", async () => {
				...
				const vesselGRAIAmount_Asset = await getOpenVesselDebtTokenAmount(vesselTotalDebt, erc20.address)
