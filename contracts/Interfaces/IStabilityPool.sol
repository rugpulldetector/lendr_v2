...
interface IStabilityPool is IDeposit {
	 ...
	 * - _amount is not zero
	 * ---
	 * - Triggers a LNDR issuance, based on time passed since the last issuance. The LNDR issuance is shared between *all* depositors.
	 * - Sends depositor's accumulated gains (LNDR, assets) to depositor
	 */
	 ...
	 * ---
	 * - Triggers a LNDR issuance, based on time passed since the last issuance. The LNDR issuance is shared between *all* depositors.
	 * - Sends all depositor's accumulated gains (LNDR, assets) to depositor
	 ...
	 */
	function getDepositorLNDRGain(address _depositor) external view returns (uint256);
