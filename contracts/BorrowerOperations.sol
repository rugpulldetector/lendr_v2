...
contract BorrowerOperations is LendrBase, ReentrancyGuardUpgradeable, UUPSUpgradeable, IBorrowerOperations {
	...
	function openVessel(
		...
		vars.ICR = LendrMath._computeCR(_assetAmount, vars.compositeDebt, vars.price);
		vars.NICR = LendrMath._computeNominalCR(_assetAmount, vars.compositeDebt);
	...
	function _adjustVessel(
		...
		assert(msg.sender == _borrower || (stakedDebtToken == msg.sender && _assetSent != 0 && _debtTokenChange == 0));
		...
		vars.oldICR = LendrMath._computeCR(vars.coll, vars.debt, vars.price);
	...
	function _getNewICRFromVesselChange(
		...
		uint256 newICR = LendrMath._computeCR(newColl, newDebt, _price);
		return newICR;
	}
	...
	function _getNewTCRFromVesselChange(
		...
		uint256 newTCR = LendrMath._computeCR(totalColl, totalDebt, _price);
		return newTCR;
	}
