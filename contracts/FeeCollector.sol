...
contract FeeCollector is IFeeCollector, UUPSUpgradeable, OwnableUpgradeable, Addresses {
	...
	bool public constant routeToLNDRStaking = false; // if true, collected fees go to stakers; if false, to the treasury
	...
	function handleRedemptionFee(address _asset, uint256 _amount) external onlyVesselManager {
		if (_routeToLNDRStaking()) {
			ILNDRStaking(lndrStaking).increaseFee_Collateral(_asset, _amount);
		}
		emit RedemptionFeeCollected(_asset, _amount);
	}

	function getProtocolRevenueDestination() public view override returns (address) {
		return _routeToLNDRStaking() ? lndrStaking : treasuryAddress;
	}
	...
	/**
	 * Transfers collected (debt token) fees to either the treasury or the LNDRStaking contract, depending on a flag.
	 */
	function _collectFee(address _borrower, address _asset, uint256 _feeAmount) internal {
		if (_feeAmount != 0) {
			...
			if (_routeToLNDRStaking()) {
				ILNDRStaking(lndrStaking).increaseFee_DebtToken(debtToken, _feeAmount);
			}
	 ...
	 */
	function _routeToLNDRStaking() internal view virtual returns (bool) {
		return routeToLNDRStaking;
	}
