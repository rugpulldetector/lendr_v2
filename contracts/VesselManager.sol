...
contract VesselManager is IVesselManager, UUPSUpgradeable, ReentrancyGuardUpgradeable, LendrBase {
	// Constants ------------------------------------------------------------------------------------------------------
	...
	function getNominalICR(address _asset, address _borrower) external view override returns (uint256) {
		...
		uint256 NICR = LendrMath._computeNominalCR(currentAsset, currentDebt);
		return NICR;
	}
	...
	function getCurrentICR(address _asset, address _borrower, uint256 _price) public view override returns (uint256) {
		(uint256 currentAsset, uint256 currentDebt) = _getCurrentVesselAmounts(_asset, _borrower);
		uint256 ICR = LendrMath._computeCR(currentAsset, currentDebt, _price);
		return ICR;
	}
	...
	function updateBaseRateFromRedemption(
		...
		newBaseRate = LendrMath._min(newBaseRate, DECIMAL_PRECISION);
	...
	function redistributeDebtAndColl(
		...
		uint256 _collToSendToStakedDebtToken
	) external override nonReentrant onlyVesselManagerOperations {
		// IStabilityPool(stabilityPool).offset(_debtToOffset, _asset, _collToSendToStakedDebtToken);
	
		IStakedDebtToken(stakedDebtToken).offset(_debtToOffset, _asset, _collToSendToStakedDebtToken);
		if (_debt == 0) {
			return;
		}
		/*
	...
	function _calcRedemptionRate(address _asset, uint256 _baseRate) internal view returns (uint256) {
		return LendrMath._min(IAdminContract(adminContract).getRedemptionFeeFloor(_asset) + _baseRate, DECIMAL_PRECISION);
	}
	...
	function _calcDecayedBaseRate(address _asset) internal view returns (uint256) {
		...
		uint256 decayFactor = LendrMath._decPow(MINUTE_DECAY_FACTOR, minutesPassed);
	...
	// --- Vessel property setters, called by Lendr's BorrowerOperations/VMRedemptions/VMLiquidations ---------------
