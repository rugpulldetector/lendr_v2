...
contract DebtToken is IDebtToken, ERC20Permit, Ownable {
	...
	// address public constant stakedDebtTokenAddress = 0x4F39F12064D83F6Dd7A2BDb0D53aF8be560356A6;
	...
	address public stakedDebtTokenAddress;
	address public vesselManagerAddress;

	function setAddresses(
		address _borrowerOperationsAddress,
		address _stakedDebtTokenAddress,
		address _vesselManagerAddress
	) public onlyOwner() {
		require(_borrowerOperationsAddress != address(0) && _stakedDebtTokenAddress != address(0) && _vesselManagerAddress != address(0), "Invalid address");
		borrowerOperationsAddress = _borrowerOperationsAddress;
		stakedDebtTokenAddress = _stakedDebtTokenAddress;
	...
	function sendToPool(address _sender, address _poolAddress, uint256 _amount) external override {
		_requireCallerIsStakedDebtToken();
	...
	function _requireCallerIsBOorVesselMorSP() internal view {
		require(
			msg.sender == borrowerOperationsAddress ||
				...
				msg.sender == stakedDebtTokenAddress,
			"DebtToken: Caller is neither BorrowerOperations nor VesselManager nor StakedDebtToken"
		);
	}

	function _requireCallerIsStakedDebtToken() internal view {
		require(msg.sender == stakedDebtTokenAddress, "DebtToken: Caller is not the StakedDebtToken");
	}

	function _requireCallerIsVesselMorSP() internal view {
		require(
			msg.sender == vesselManagerAddress || msg.sender == stakedDebtTokenAddress,
			"DebtToken: Caller is neither VesselManager nor StakedDebtToken"
		);
	}
}
