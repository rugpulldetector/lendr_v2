 ...
 *
 *
 * --- Lendr ISSUANCE TO STABILITY POOL DEPOSITORS ---
 *
 * An Lendr issuance event occurs at every deposit operation, and every liquidation.
 *
 * All deposits earn a share of the issued Lendr in proportion to the deposit as a share of total deposits.
 *
 * Please see the system Readme for an overview:
 * https://github.com/liquity/dev/blob/main/README.md#lqty-issuance-to-stability-providers
 *
 * We use the same mathematical product-sum approach to track Lendr gains for depositors, where 'G' is the sum corresponding to Lendr gains.
 * The product P (and snapshot P_t) is re-used, as the ratio P/P_t tracks a deposit's depletion due to liquidations.
 *
 */
contract StabilityPool is ReentrancyGuardUpgradeable, UUPSUpgradeable, LendrBase, IStabilityPool {
	 ...
	 * depositSnapshots are used to allocate LNDR rewards, calculate compoundedDepositAmount
	...
	/*
	 * Similarly, the sum 'G' is used to calculate LNDR gains. During it's lifetime, each deposit d_t earns a LNDR gain of
	 *  ( d_t * [G - G_t] )/P_t, where G_t is the depositor's snapshot of G taken at time t when  the deposit was made.
	 *
	 *  LNDR reward events occur are triggered by depositor operations (new deposit, topup, withdrawal), and liquidations.
	 *  In each case, the LNDR reward is issued (i.e. G is updated), before other state changes are made.
	 */
	mapping(uint128 => mapping(uint128 => uint256)) public epochToScaleToG;

	// Error tracker for the error correction in the LNDR issuance calculation
	uint256 public lastLNDRError;
	...
	function provideToSP(uint256 _amount, address[] calldata _assets) external override nonReentrant {
		...
		// First pay out any LNDR gains
		_payOutLNDRGains(msg.sender);
	...
	*/
	...
	function _withdrawFromSP(
		...
		uint256 debtTokensToWithdraw = LendrMath._min(_amount, compoundedDeposit);
		uint256 loss = initialDeposit - compoundedDeposit; // Needed only for event log
	...
	// --- LNDR issuance functions ---

	function _triggerLNDRIssuance() internal {
		if (communityIssuance != address(0)) {
			uint256 LNDRIssuance = ICommunityIssuance(communityIssuance).issueLNDR();
			_updateG(LNDRIssuance);
		}
	}

	function _updateG(uint256 _LNDRIssuance) internal {
		uint256 cachedTotalDebtTokenDeposits = totalDebtTokenDeposits; // cached to save an SLOAD
		/*
		 * When total deposits is 0, G is not updated. In this case, the LNDR issued can not be obtained by later
		 * depositors - it is missed out on, and remains in the balanceof the CommunityIssuance contract.
		 *
		 */
		if (cachedTotalDebtTokenDeposits == 0 || _LNDRIssuance == 0) {
			return;
		}
		uint256 LNDRPerUnitStaked = _computeLNDRPerUnitStaked(_LNDRIssuance, cachedTotalDebtTokenDeposits);
		uint256 marginalLNDRGain = LNDRPerUnitStaked * P;
		uint256 newEpochToScaleToG = epochToScaleToG[currentEpoch][currentScale];
		newEpochToScaleToG += marginalLNDRGain;
		epochToScaleToG[currentEpoch][currentScale] = newEpochToScaleToG;
		emit G_Updated(newEpochToScaleToG, currentEpoch, currentScale);
	}

	function _computeLNDRPerUnitStaked(uint256 _LNDRIssuance, uint256 _totalDeposits) internal returns (uint256) {
		/*
		 * Calculate the LNDR-per-unit staked.  Division uses a "feedback" error correction, to keep the
		 ...
		 */
		uint256 LNDRNumerator = (_LNDRIssuance * DECIMAL_PRECISION) + lastLNDRError;
		uint256 LNDRPerUnitStaked = LNDRNumerator / _totalDeposits;
		lastLNDRError = LNDRNumerator - (LNDRPerUnitStaked * _totalDeposits);
		return LNDRPerUnitStaked;
	}
	...
	/*
	 * Calculate the LNDR gain earned by a deposit since its last snapshots were taken.
	 * Given by the formula:  LNDR = d0 * (G - G(0))/P(0)
	 * where G(0) and P(0) are the depositor's snapshots of the sum G and product P, respectively.
	 * d0 is the last recorded deposit value.
	 */
	function getDepositorLNDRGain(address _depositor) public view override returns (uint256) {
		uint256 initialDeposit = deposits[_depositor];
		if (initialDeposit == 0) {
			return 0;
		}
		...
		Snapshots storage snapshots = depositSnapshots[_depositor];
		return _getLNDRGainFromSnapshots(initialDeposit, snapshots);
	}

	function _getLNDRGainFromSnapshots(
		uint256 initialStake,
		Snapshots storage snapshots
	) internal view returns (uint256) {
		/*
		 * Grab the sum 'G' from the epoch at which the stake was made. The LNDR gain may span up to one scale change.
		 * If it does, the second portion of the LNDR gain is scaled by 1e9.
		 * If the gain spans no scale change, the second portion will be 0.
		 */
		...
		uint256 LNDRGain = (initialStake * (firstPortion + secondPortion)) / P_Snapshot / DECIMAL_PRECISION;
	...
	function _payOutLNDRGains(address _depositor) internal {
		if (address(communityIssuance) != address(0)) {
			uint256 depositorLNDRGain = getDepositorLNDRGain(_depositor);
			ICommunityIssuance(communityIssuance).sendLNDR(_depositor, depositorLNDRGain);
			emit LNDRPaidToDepositor(_depositor, depositorLNDRGain);
		}
	}
