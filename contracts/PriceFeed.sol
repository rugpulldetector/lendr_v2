...
contract PriceFeed is IPriceFeed, OwnableUpgradeable, UUPSUpgradeable, Addresses {
	...
	address	 public 			rwaPriceFeedAddress;
	...
	function setRWAPriceFeed(
		address _rwaPriceFeedAddress
	) public onlyOwner() {
		require(_rwaPriceFeedAddress != address(0), "Invalid address");

		rwaPriceFeedAddress = _rwaPriceFeedAddress;
	}
	...
	function fetchPrice(address _asset) public view virtual returns (uint256) {
		require(rwaPriceFeedAddress != address(0), "Not initialized");

        uint256 rwaPrice = IRWAPriceFeed(rwaPriceFeedAddress).getRWAPrice(debtToken);
        return fetchPriceAbsolute(_asset) * DECIMAL_PRECISION / rwaPrice;
	}

	function fetchPriceAbsolute(address _asset) public view virtual returns (uint256) {
		// Tries fetching the price from the oracle
		OracleRecordV2 memory oracle = oracles[_asset];
		...
		oracle = fallbacks[_asset];
		price = _fetchOracleScaledPrice(oracle);
		if (price != 0) {
			return oracle.isEthIndexed ? _calcEthIndexedPrice(price) : price;
		}
		revert PriceFeed__InvalidOracleResponseError(_asset);
	}
	...
	/**
	 * @dev Scales oracle's response up/down to Lendr's target precision; returns unaltered price if already on
	 *     target digits.
	 */
