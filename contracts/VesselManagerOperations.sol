...
contract VesselManagerOperations is IVesselManagerOperations, UUPSUpgradeable, ReentrancyGuardUpgradeable, LendrBase {
	...
	function liquidateVessels(address _asset, uint256 _n) external override nonReentrant {
		...
		vars.debtTokenInStabPool = IERC4626Upgradeable(stakedDebtToken).totalAssets();
		...
		while (currentVesselBorrower != address(0) && remainingDebt != 0 && vars.maxIterations-- != 0) {
			...
			if (currentVesselNetDebt <= remainingDebt) {
				...
				if (currentVesselNetDebt > IAdminContract(adminContract).getMinNetDebt(vars.asset)) {
					uint256 maxRedeemableDebt = LendrMath._min(
						remainingDebt,
					...
					partialRedemptionHintNewICR = LendrMath._computeNominalCR(newColl, compositeDebt);
    ...
    */
	function getApproxHint(
		...
		diff = LendrMath._getAbsoluteDifference(_CR, IVesselManager(vesselManager).getNominalICR(_asset, hintAddress));
		...
		while (i < _numTrials) {
			...
			uint256 currentDiff = LendrMath._getAbsoluteDifference(currentNICR, _CR);
	...
	function computeNominalCR(uint256 _coll, uint256 _debt) external pure override returns (uint256) {
		return LendrMath._computeNominalCR(_coll, _debt);
	}
	...
	function _getTotalFromBatchLiquidate_RecoveryMode(
		...
		for (vars.i = 0; vars.i < _vesselArray.length; ) {
			...
			if (!vars.backToNormalMode) {
				...
				uint256 TCR = LendrMath._computeCR(vars.entireSystemColl, vars.entireSystemDebt, _price);
		...
		returns (uint256 debtToOffset, uint256 collToSendToSP, uint256 debtToRedistribute, uint256 collToRedistribute)
	{
		if (_debtTokenInStabPool != 0) {
			 ...
			 *
			 */
			debtToOffset = LendrMath._min(_debt, _debtTokenInStabPool);
	...
	function _checkPotentialRecoveryMode(
		...
		uint256 TCR = LendrMath._computeCR(_entireSystemColl, _entireSystemDebt, _price);
	...
	function _redeemCollateralFromVessel(
		...
		singleRedemption.debtLot = LendrMath._min(
			_maxDebtTokenAmount,
		...
		if (newDebt == IAdminContract(adminContract).getDebtTokenGasCompensation(_asset)) {
			...
			uint256 newNICR = LendrMath._computeNominalCR(newColl, newDebt);
	...
	function setRedemptionSofteningParam(uint256 _redemptionSofteningParam) public onlyOwner {
