...
contract ActivePool is OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable, IActivePool, Addresses {
	...
	modifier callerIsBorrowerOpsOrDefaultPool() {
		require(
			...
			"ActivePool: Caller is not an authorized Lendr contract"
		);
		_;
	}
	...
	modifier callerIsBorrowerOpsOrStakedDebtTokenOrVesselMgr() {
		require(
			msg.sender == borrowerOperations || msg.sender == stakedDebtToken || msg.sender == vesselManager,
			"ActivePool: Caller is not an authorized Lendr contract"
		);
		_;
	}

	modifier callerIsBorrowerOpsOrStakedDebtTokenOrVesselMgrOrVesselMgrOps() {
		require(
			msg.sender == borrowerOperations ||
				msg.sender == stakedDebtToken ||
	...
	function decreaseDebt(
		address _asset,
		uint256 _amount
	) external override callerIsBorrowerOpsOrStakedDebtTokenOrVesselMgr {
	...
	function sendAsset(
		...
		address _account,
		uint256 _amount
	) external override nonReentrant callerIsBorrowerOpsOrStakedDebtTokenOrVesselMgrOrVesselMgrOps {
	...
	function isERC20DepositContract(address _account) private view returns (bool) {
		return (_account == defaultPool || _account == collSurplusPool || _account == stakedDebtToken);
	}
